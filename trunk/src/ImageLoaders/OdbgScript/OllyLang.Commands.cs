...
{
	if(args.Length == 1 && is_variable(args[0]))
	{
...
rulong maxsize = 0;
	...
	if(args.Length >= 2 && args.Length <= 3 && GetRulong(args[0],  out addr))
	{
		...
		{
			finddata = Helper.rul2hexstr(reverse(dw));
			...
			finddata = finddata.resize(end + (end % 2), '0');
		}
		...
		if(Helper.is_hexwild(finddata))
		{
            ...
            if (Host.TE_GetMemoryInfo(addr, out MemInfo))
            {
                ...
                if (Host.TE_ReadMemory(addr, memlen, membuf))
                {
...
string str;
	...
	if(args.Length == 1)
	{
		...
		{
			string name = Helper.filefrompath(Host.TE_GetTargetPath());
			int offs;
			if((offs = name.LastIndexOf('.')) >= 0)
				name = name.Remove(offs);
...
rulong dw = 0;
	...
	if(args.Length >= 1 && args.Length <= 3 && is_variable(args[0]))
	{
		switch(args.Length)
		{
		case 3:
...
if (args.Length >= 3 && args.Length <= 4 && GetRulong(args[0], out addr) && Helper.is_bytestring(args[1]) && Helper.is_bytestring(args[2]))
	{
        ...
        byte[] membuf = null, mask1 = null, mask2 = null, bytes1 = null, bytes2;
			...
			membuf = new byte[len];
			if(Host.TE_ReadMemory(addr, len, membuf))
			{
				bool replaced = false;
...
private bool DoTICK(string [] args)
{
	...
	if(args.Length >= 0 && args.Length <= 2)
	{
		...
		else if(is_variable(args[0]) || DoVAR(args[0]))
		{
