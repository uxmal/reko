...
```
dotnet build -p:Configuration=UnixRelease -p:Platform=x64 src/Reko-decompiler.slnx
```
...
.NET SDK 9.0 and C# 12 (or later). Your C++ compiler must be C++ 14 compliant. The .NET SDK is
[available from Microsoft](https://dotnet.microsoft.com/en-us/download/dotnet/9.0).
...
x86 and 64-bit executables for x86-64, and 64-bit executables for ARM64 processors. The names of the solution platforms are `x86`, `x64`, and `ARM64`, following 
...
C++ projects will depend on this project indirectly, via header files generated by `hdrgen` (see below).
`Core.csproj` is built platform neutrally, using the `Any CPU` platform.
...
Separate solution build platforms are used for the supported `x86`, `x86-64`, and `ARM64` execution platforms. These solution platforms are called `x86`, `x64`, and `ARM64`, respectively.
...
Each one of the front end drivers is compiled for each supported solution platform: `x86`, `x64`, and `ARM64`.

### Unit tests

The Reko unit test suite is compiled for each supported solution platform: `x86`, `x64`, and `ARM64`.
