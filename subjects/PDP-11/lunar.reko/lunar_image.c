// lunar_image.c
// Generated by decompiling lunar.lda
// using Reko decompiler version 0.11.6.0.

#include "lunar.h"

word16 g_w0000 = 0x5F; // 0000
word16 g_w0004 = 0x0200; // 0004
word16 g_w0006 = 0xE0; // 0006
struct Eq_n g_t000A = // 000A
	{
		&g_w0202,
	};
struct Eq_n g_t000C = // 000C
	{
		&g_w00E0,
	};
struct Eq_n g_t003C = // 003C
	{
		null,
	};
struct Eq_n g_t003F = // 003F
	{
		null,
	};
struct Eq_n * g_ptr0044 = null; // 0044
int16 g_w0046 = 0; // 0046
struct Eq_n * g_ptr0048 = null; // 0048
struct Eq_n * g_ptr004A = null; // 004A
struct Eq_n * g_ptr004C = null; // 004C
struct Eq_n * g_ptr004E = null; // 004E
struct Eq_n * g_ptr0050 = null; // 0050
struct Eq_n * g_ptr0052 = null; // 0052
int16 g_w0054 = 0; // 0054
struct Eq_n * g_ptr0056 = null; // 0056
ci16 g_w0058 = 0x00; // 0058
struct Eq_n * g_ptr005A = null; // 005A
word16 g_w005C = 0x00; // 005C
word16 g_w005E = 0x00; // 005E
word16 g_w005F = 0x00; // 005F
struct Eq_n * g_ptr0060 = null; // 0060
struct Eq_n * g_ptr0062 = null; // 0062
struct Eq_n * g_ptr0064 = null; // 0064
struct Eq_n * g_ptr0066 = null; // 0066
struct Eq_n * g_ptr0068 = null; // 0068
word16 g_w006A = 0x00; // 006A
struct Eq_n * g_ptr006C = null; // 006C
struct Eq_n * g_ptr006E = null; // 006E
cu16 g_w0070 = 0x00; // 0070
int16 g_w0072 = 0; // 0072
cui16 g_w0074 = 0x00; // 0074
int16 g_w0078 = 0; // 0078
word16 g_w007A = 0x00; // 007A
ci16 g_w007E = 0x00; // 007E
word16 g_w0080 = 0x00; // 0080
word16 * g_ptr0082 = null; // 0082
word16 g_w0084 = 0x00; // 0084
word16 * g_ptr0086 = null; // 0086
cui16 g_w008A = 0x00; // 008A
cui16 g_w008C = 0x00; // 008C
cui16 g_w008E = 0x00; // 008E
cui16 g_w0090 = 0x00; // 0090
struct Eq_n * g_ptr0092 = null; // 0092
ci16 g_w00A0 = 0x00; // 00A0
ci16 g_w00A2 = 0x00; // 00A2
ci16 g_w00A4 = 0x00; // 00A4
cui16 g_w00A6 = 0x00; // 00A6
cui16 g_w00A8 = 0x00; // 00A8
wchar_t g_w00AA = L'\0'; // 00AA
int16 g_w00AC = 0; // 00AC
ci16 g_w00AE = 0x00; // 00AE
cui16 g_w00B0 = 0xB0; // 00B0
word16 g_w00E0 = 6186; // 00E0
// 0128: void fn0128()
// Called from:
//      fn13AA
void fn0128()
{
	__wait();
	struct Eq_n * r0_n = g_ptr006C;
	if (r0_n != null)
	{
		g_ptr006C = null;
		g_ptr006E = r0_n;
		fn053A();
	}
}

word16 * g_a013E[] = // 013E
	{
	};
word16 g_w015E = 0x5E; // 015E
word16 g_w0202 = 0x00; // 0202
Eq_n g_t021C = // 021C
	{
		fn34E0,
	};
// 0242: void fn0242(Register word16 r0, Register (ptr16 word16) r1)
// Called from:
//      fn03CE
//      fn053A
//      fn0856
//      fn103C
void fn0242(word16 r0, word16 * r1)
{
	word16 * r3_n;
	g_w0080 = 0x00;
	g_ptr0082 = null;
	g_ptr0086 = (word16 *) 0x7FFF;
	word16 r4_n = r0;
	word16 * r5_n = r1;
	while (true)
	{
		cui16 wLoc02_n = 0x00;
		word16 r2_n = fn02C8(r4_n + 0x02, out r3_n, out r4_n);
		ci16 r2_n = r2_n - g_w0080;
		if (r2_n <= 0x00)
		{
			r2_n = -r2_n;
			wLoc02_n = 0x2000;
		}
		cu16 r2_n = __swab(r2_n & 0x3F);
		g_w0080 = r2_n;
		ci16 r3_n = r3_n - g_ptr0082;
		if (r3_n <= 0x00)
		{
			r3_n = -r3_n;
			wLoc02_n |= 0x40;
		}
		g_ptr0082 = r3_n;
		cui16 r3_n = r3_n & 0x3F;
		if (r3_n < g_ptr0086)
			g_ptr0086 = r3_n;
		*r5_n = r3_n | r2_n >> 0x01 | wLoc02_n;
		++r5_n;
	}
}

// 02C8: Register word16 fn02C8(Register (ptr16 Eq_n) r4, Register out Eq_n r3Out, Register out ptr16 r4Out)
// Called from:
//      fn0242
word16 fn02C8(struct Eq_n * r4, union Eq_n & r3Out, ptr16 & r4Out)
{
	word16 r2_n;
	fn125E((int16) r4->b0000, g_ptr004A, out r2_n);
	word16 r2_n;
	fn125E((int16) r4->b0001, g_ptr004C, out r2_n);
	word16 r2_n;
	fn125E((int16) r4->b0000, g_ptr004C, out r2_n);
	word16 r2_n;
	fn125E((int16) r4->b0001, g_ptr004A, out r2_n);
	r3Out = r2_n - r2_n;
	r4Out = &r4->b0001 + 1;
	return r2_n + r2_n;
}

// 0300: void fn0300(Register (ptr16 Eq_n) r4)
// Called from:
//      fn053A
void fn0300(struct Eq_n * r4)
{
	if (g_ptr0068 == null)
	{
		g_w25C0 = 0xF700;
		g_ptr0066 = null;
		g_w006A = 14300;
		g_w006A = g_ptr0068 + g_w006A;
		g_ptr004E = null;
		g_ptr0050 = null;
		g_ptr0052 = (struct Eq_n *) 62866;
	}
	else
	{
		ui32 r2_r3_n;
		fn114A(g_ptr0064, &g_t2904, out r2_r3_n);
		struct Eq_n * r3_n = (struct Eq_n *) <invalid>;
		struct Eq_n * r2_n = (struct Eq_n *) <invalid>;
		struct Eq_n * r3_n;
		fn126C(&g_ptr0064, r2_n, r3_n, out r3_n);
		g_ptr0066 = r3_n;
		ui32 r2_r3_n;
		fn114A(g_ptr006E, r3_n, out r2_r3_n);
		struct Eq_n * r3_n = (struct Eq_n *) <invalid>;
		struct Eq_n * r2_n = (struct Eq_n *) <invalid>;
		word16 r3_n;
		fn126C((char *) &g_t021C + 960, r2_n, r3_n, out r3_n);
		ci16 r3_n = r3_n - g_ptr0068;
		struct Eq_n * r3_n;
		if (r3_n >= 0x00)
		{
			g_w25C0 = 0xF700;
			r3_n = null;
		}
		else
		{
			r3_n = -r3_n;
			if (g_w25C0 != 0x00 && r3_n < &g_t07D0)
			{
				g_w25C0 = 0x00;
				*(word16 *) 62466 = *(word16 *) 62466;
			}
		}
		g_ptr0068 = r3_n;
		word16 r3_n;
		fn126C(&g_t000A, null, r3_n, out r3_n);
		g_w006A = r3_n + 14300;
		ui32 r2_r3_n;
		fn114A(g_ptr0066, (struct Eq_n *) 0x3ED7, out r2_r3_n);
		struct Eq_n * r3_n = (struct Eq_n *) <invalid>;
		struct Eq_n * r2_n = (struct Eq_n *) <invalid>;
		struct Eq_n * r3_n;
		fn126C(r4, r2_n, r3_n, out r3_n);
		g_ptr004E = r3_n;
		struct Eq_n * r2_n;
		fn125E(r3_n, g_ptr004A, out r2_n);
		g_ptr0050 = r2_n;
		word16 r2_n;
		fn125E(g_ptr004E, g_ptr004C, out r2_n);
		g_ptr0052 = r2_n - 0x0A6E;
	}
}

// 03CE: void fn03CE(Register (ptr16 word16) r0)
// Called from:
//      fn053A
void fn03CE(word16 * r0)
{
	int16 r2_n = (int16) (g_ptr0064 >> 3)->b2766;
	++g_w008A;
	int16 r2_n = r2_n + (int16) (g_w008A & 0x1F)->b2773;
	g_w008C += r2_n;
	ci16 r4_n;
	struct Eq_n * r5_n = (struct Eq_n *) g_a27B0;
	byte * r3_n = (g_w008C & 0x03) + 10131;
	for (r4_n = 0x0C; r4_n > 0x00; --r4_n)
	{
		r5_n->b0000 = *r3_n;
		byte * r5_n = (byte *) (&r5_n->b0000 + 1);
		*r5_n = (byte) r2_n;
		++r3_n;
		r5_n = (struct Eq_n *) (r5_n + 7);
	}
	++g_w0090;
	g_w0090 &= 0x03;
	g_w008E += 0x0180;
	g_w008E &= 0x0380;
	g_w27A8 = 0x8C54;
	g_w27A8 |= g_w0090;
	g_w27A8 |= g_w008E;
	fn0242(0x27A2, r0);
}

// 0444: void fn0444()
// Called from:
//      fn053A
//      fn0856
//      fn0E98
//      fn103C
void fn0444()
{
	int16 r0_n = g_w0046;
	if (r0_n <= 0x00)
	{
		while (r0_n <= 65356)
		{
			r0_n += 0x0168;
			if (r0_n > 0x00)
				break;
		}
	}
	else
	{
		while (r0_n > 0xB4)
		{
			r0_n -= 0x0168;
			if (r0_n >= 0x00)
				break;
		}
	}
	g_w0046 = r0_n;
	if (r0_n <= 0x00)
		r0_n += 0x0168;
	g_ptr004A = (struct Eq_n *) *((char *) g_a31DC + r0_n * 0x02);
	struct Eq_n * r0_n = r0_n << 1;
	if (r0_n >= &g_t021C)
		r0_n -= 0x02D0;
	g_ptr004C = r0_n->ptr3290;
}

// 0488: void fn0488()
// Called from:
//      fn053A
void fn0488()
{
	word16 r3_n;
	word16 r3_n;
	struct Eq_n * r1_n = g_ptr0050;
	if (r1_n <= null)
		r1_n = -r1_n;
	ui32 r2_r3_n;
	fn114A(g_ptr006E, r1_n, out r2_r3_n);
	struct Eq_n * r3_n = (struct Eq_n *) <invalid>;
	struct Eq_n * r2_n = (struct Eq_n *) <invalid>;
	ci16 r3_n;
	fn126C(&g_t0BB8, r2_n, r3_n, out r3_n);
	if (g_ptr0050 <= null)
		r3_n = -r3_n;
	struct Eq_n * r3_n = (r3_n >> 1) + g_w0054;
	g_w0054 += r3_n;
	struct Eq_n * r1_n = r3_n;
	if (r3_n <= null)
		r1_n = -r3_n;
	ui32 r2_r3_n;
	fn114A(g_ptr006E, r1_n, out r2_r3_n);
	uint32 r2_r3_n = r2_r3_n + (uint32) g_ptr0056;
	struct Eq_n * r2_n = fn126C((char *) &g_t021C + 60, SLICE(r2_r3_n, word16, 16), (word16) r2_r3_n, out r3_n);
	if (r3_n <= null)
		r3_n = -r3_n;
	g_w005C += r3_n;
	g_ptr0056 = r2_n;
	struct Eq_n * r1_n = g_ptr0052;
	if (r1_n <= null)
		r1_n = -r1_n;
	ui32 r2_r3_n;
	fn114A(g_ptr006E, r1_n, out r2_r3_n);
	struct Eq_n * r3_n = (struct Eq_n *) <invalid>;
	struct Eq_n * r2_n = (struct Eq_n *) <invalid>;
	ci16 r3_n;
	fn126C(&g_t0BB8, r2_n, r3_n, out r3_n);
	if (g_ptr0052 <= null)
		r3_n = -r3_n;
	struct Eq_n * r3_n = (r3_n >> 1) + g_w0058;
	g_w0058 += r3_n;
	struct Eq_n * r1_n = r3_n;
	if (r3_n <= null)
		r1_n = -r3_n;
	ui32 r2_r3_n;
	fn114A(g_ptr006E, r1_n, out r2_r3_n);
	uint32 r2_r3_n = r2_r3_n + (uint32) g_ptr005A;
	struct Eq_n * r2_n = fn126C((char *) &g_t021C + 60, SLICE(r2_r3_n, word16, 16), (word16) r2_r3_n, out r3_n);
	if (r3_n <= null)
		r3_n = -r3_n;
	g_w005E += r3_n;
	g_ptr005A = r2_n;
}

// 053A: void fn053A()
// Called from:
//      fn0128
void fn053A()
{
	word16 r3_n;
	g_w0084 = ~0x3F;
	struct Eq_n * r5_n = g_ptr0044;
	g_ptr0044 = null;
	struct Eq_n * r0_n = g_ptr006E;
	struct Eq_n * r1_n = r5_n;
	if (r5_n <= null)
		r1_n = -r5_n;
	ui32 r2_r3_n;
	fn114A(r0_n, r1_n, out r2_r3_n);
	uint32 r2_r3_n = r2_r3_n + (uint32) g_ptr0048;
	struct Eq_n * r2_n = fn126C(&g_t003C, SLICE(r2_r3_n, word16, 16), (word16) r2_r3_n, out r3_n);
	if (r5_n <= null)
		r3_n = -r3_n;
	g_w0046 += r3_n;
	g_ptr0048 = r2_n;
	fn0444();
	g_ptr0092 = (struct Eq_n *) ((char *) g_ptr0092 + 2);
	fn0242(9944, (g_ptr0092 & 0x02)->ptr34AC);
}

// 0790: void fn0790()
// Called from:
//      fn053A
//      fn0856
void fn0790()
{
}

word16 g_w07BA = 0x00; // 07BA
struct Eq_n g_t07D0 = // 07D0
	{
		&g_w005F,
	};
// 0856: void fn0856(Register ci16 r4)
// Called from:
//      fn053A
void fn0856(ci16 r4)
{
l0856:
	fn1578();
	ci16 r5_n = g_w007E;
	if (r5_n >= 0x00)
	{
		if (r5_n > 0x03)
		{
			r4 = g_w0058;
			if (r4 >= 64936)
			{
				if (r4 >= 65236)
				{
					if (r4 >= ~0x95)
					{
						if (g_w34CA == 8458)
							g_w34CA = 0x00;
					}
					else
						g_w34CA = 8458;
				}
				else
					g_w34CA = 8408;
			}
			else
				g_w34CA = 0x20AC;
l0888:
			int16 r0_n = g_w0078;
			fn0CCA(r0_n);
			switch (r0_n)
			{
			case 0:
				fn0790();
				return;
			case 1:
				if (r5_n > 0x1A)
				{
					fn0790();
					return;
				}
				ci16 v68_n = r4 - 64936;
				if (v68_n <= 0x00)
				{
					if (v68_n <= 0x00)
						goto l0904;
					g_w34CA = 0x23FE;
					fn0E32();
					goto l0856;
				}
				else
				{
					g_w34CA = 0x237E;
					word16 wLoc02_n = 0x03;
					if (g_w0054 >= 0x00)
						wLoc02_n = 0x04;
					fn0C90(r0_n, (byte) wLoc02_n);
					g_w34B6 -= 0x10;
					fn0F04(r0_n);
				}
			}
		}
	}
	else if (r5_n <= ~0x09)
		goto l0904;
	g_ptr0060 = null;
	g_w25AA = 0x01C2;
	g_w34C2 = 0x00;
	g_w34D2 = 0x00;
	if (r5_n != 0x00 && r5_n > 0x00)
		goto l0888;
	ci16 r4_n = g_w0058;
	if (r4_n > 64936)
	{
		word16 r0_n;
		if (r4_n > 65236)
		{
			if (r4_n > ~0x95)
			{
				if (r4_n > ~0x4F)
					r0_n = 0x214E;
				else
					r0_n = 8566;
			}
			else
				r0_n = 0x21A4;
		}
		else
			r0_n = 0x21CE;
		g_w34C2 = 0x00;
		g_w34CA = r0_n;
		int16 r0_n;
		word16 r1_n;
		if (g_w0054 > 100 || g_w0054 < ~0x63)
		{
l0964:
			r0_n = g_w0054;
			r1_n = 0x22C6;
			goto l097C;
		}
		else
		{
			if (g_w0046 >= ~0x0E && g_w0046 <= 0x0F)
			{
				int16 r1_n = g_w0078;
				r0_n = g_a28F2[r1_n] - (&g_w28F0)[r1_n];
				int16 r2_n = r0_n;
				if (r0_n <= 0x00)
					r2_n = -r0_n;
				if (r2_n < 0x30)
				{
					fn0B06();
					goto l0964;
				}
				r1_n = 8980;
			}
			else
			{
				r0_n = g_w0046;
				r1_n = 0x2258;
			}
l097C:
			g_w34C2 = r1_n;
			int16 r1_n = g_w0078;
			word16 wLoc02_n = 0x03;
			if (r0_n >= 0x00)
				wLoc02_n = 0x04;
			fn0C90(r1_n, (byte) wLoc02_n);
			ui16 r2_n = g_a28F2[r1_n] - (&g_w28F0)[r1_n];
			ci16 r2_n = r2_n - (r2_n * 0x03 >> 2);
			struct Eq_n * sp_n = (struct Eq_n *) <invalid>;
			ci16 r2_n = r2_n + (r2_n >> 1);
			if (r2_n <= 0x00)
			{
				if (r2_n < ~0x2C)
					r2_n = ~0x2C;
			}
			else if (r2_n > 0x2D)
				r2_n = 0x2D;
			int16 r3_n = 0x5A;
			if (r0_n <= 0x00)
				r3_n = -90;
			g_w0046 = r2_n + r3_n;
			struct Eq_n * r1_n = (char *) g_ptr0092 + 2 & 0x02;
			g_ptr0092 = r1_n;
			word16 * r1_n = r1_n->ptr34AC;
			sp_n->ptrFFFFFFFE = r1_n;
			fn0444();
			fn0242(9944, r1_n);
		}
	}
l0904:
	g_w34CA = 0x2212;
	fn0F04(fn0E06(0x20));
}

// 0A0A: void fn0A0A()
// Called from:
//      fn053A
void fn0A0A()
{
	if (g_w007E < 0x96)
	{
		struct Eq_n * r4_n = g_ptr0064;
		Eq_n NZVC_n;
		NZVC_n.u1 = cond(r4_n - &g_t003F);
		if (r4_n > &g_t003F)
			r4_n = (struct Eq_n *) &g_t003F;
		struct Eq_n * r4_n = __rcl<word16,byte>(r4_n, 0x01, NZVC_n & 0x01);
		struct Eq_n * r4_n = __rcl<word16,byte>(r4_n, 0x01, (r4_n & 0x8000) != 0x00);
		g_w35CA = __rcl<word16,byte>(__rcl<word16,byte>(r4_n, 0x01, (r4_n & 0x8000) != 0x00), 0x01, (r4_n & 0x8000) != 0x00) & 0x0380 | 0x9C50;
		if (g_w0046 <= 0x2D && g_w0046 >= ~0x2C)
		{
			struct Eq_n * r1_n = g_ptr004A;
			if (r1_n <= null)
				r1_n = -r1_n;
			struct Eq_n * r0_n = g_w34B6 - g_w007A;
			ui32 r2_r3_n;
			fn114A(r0_n, r1_n, out r2_r3_n);
			struct Eq_n * r3_n = (struct Eq_n *) <invalid>;
			struct Eq_n * r2_n = (struct Eq_n *) <invalid>;
			word16 r3_n;
			fn126C(g_ptr004C, r2_n, r3_n, out r3_n);
			ci16 r4_n = r0_n + r3_n;
			if (g_ptr004A >= null)
				r3_n = -r3_n;
			g_w35CC = r3_n + g_w34B4;
			g_w35CE = g_w007A;
			g_w35D0 = 0xB000;
			word16 * r5_n = g_a35D2;
			if (r4_n <= 0x96)
			{
				struct Eq_n * r4_n = 0x96 - r4_n;
				ui32 r2_r3_n;
				fn114A(r4_n, g_ptr0064, out r2_r3_n);
				struct Eq_n * r3_n = (struct Eq_n *) <invalid>;
				if (r3_n >> 4 != 0x00)
				{
					int16 wLoc02_n = r3_n >> 4;
					if (r3_n >> 4 > 600)
						wLoc02_n = 600;
					cui16 r2_n = g_w00B0;
					cui16 r2_n;
					struct Eq_n * r5_n;
					int16 v53_n;
					do
					{
						struct Eq_n * r2_n = r2_n + g_w0070 + 0x01 & 0x3F;
						r2_n = r2_n + g_ptr0052;
						r4_n = ~r4_n & 0x2000 | 0x4000;
						cui16 r0_n = __swab((int16) r2_n->b2766 & 0x3F) >> 0x01 | r4_n | (int16) (r2_n & 0x3F)->b2766 & 0x3F;
						*r5_n = r0_n;
						r5_n = (struct Eq_n *) (r5_n + 1);
						r5_n->w0000 = r0_n + 0x2040 & 0x3FFF;
						v53_n = wLoc02_n - 0x01;
						r2_n = r2_n & 0x3F;
						r5_n = &r5_n->w0002;
						wLoc02_n = v53_n;
					} while (v53_n > 0x00);
					r5_n->w0002 = 0xF700;
					r5_n->w0004 = 0x00;
					g_w00B0 = r2_n & 0x3F;
					g_w34D2 = 13770;
					return;
				}
			}
		}
	}
	g_w34D2 = 0x00;
}

// 0B06: void fn0B06()
// Called from:
//      fn0856
void fn0B06()
{
	// Failed to bind call argument.
	// Please report this issue at https://github.com/uxmal/reko
	Eq_n stackArg0;
	stackArg0.u0 = <invalid>;
	fn13AA(stackArg0);
}

struct Eq_n g_t0BB8 = // 0BB8
	{
		&g_w0060,
	};
// 0C90: void fn0C90(Stack int16 wArg02, Stack byte bArg04)
// Called from:
//      fn0856
//      fn0E06
void fn0C90(int16 wArg02, byte bArg04)
{
	word16 wArg04;
	ui8 bArg04_n = (byte) wArg04;
	if (wArg02 >> 1 < 0x00)
	{
		(wArg02 >> 1)->b3013 = (wArg02 >> 1)->b3013 & 0x0F;
		bArg04_n = (byte) wArg04 << 0x04;
	}
	else
		(wArg02 >> 1)->b3013 = (wArg02 >> 1)->b3013 & 0xF0;
	(wArg02 >> 1)->b3013 = (wArg02 >> 1)->b3013 | bArg04_n;
}

// 0CCA: void fn0CCA(Stack int16 wArg02)
// Called from:
//      fn0856
void fn0CCA(int16 wArg02)
{
}

// 0CEC: void fn0CEC(Register int16 r0)
// Called from:
//      fn053A
void fn0CEC(int16 r0)
{
	<anonymous> * ptrLoc02;
	struct Eq_n * r5_n = fn0D3C(r0, ptrLoc02);
	word16 * r4_n = (g_w28F0 >> 5) + 0x17;
	r5_n->ptr0000 = r4_n;
	g_ptr0082 = r4_n;
	r5_n->w0002 = 0x8C50;
	cui16 * r5_n = (cui16 *) (&r5_n->w0002 + 1);
	struct Eq_n * r0_n = (struct Eq_n *) &g_w28F0;
	while (true)
	{
		++r0_n;
		r5_n = fn0D78((r0_n->w0000 >> 5) + 0x17, r5_n);
	}
}

// 0D3C: Register word16 fn0D3C(Register int16 r0, Stack (ptr16 code) ptrArg00)
// Called from:
//      fn0CEC
//      fn0F04
word16 fn0D3C(int16 r0, <anonymous> * ptrArg00)
{
	g_w00A0 = 225;
	g_w34DA = 0x00;
	g_w07BA = 0x00;
	g_a182A[0] = 38996;
	g_w182C = 0xF0A0;
	g_w182E = 0x00;
	ptrArg00();
	return 0x1830;
}

// 0D66: void fn0D66()
void fn0D66()
{
}

// 0D78: Register (ptr16 cui16) fn0D78(Register (ptr16 word16) r4, Register (ptr16 cui16) r5)
// Called from:
//      fn0CEC
//      fn0F04
cui16 * fn0D78(word16 * r4, cui16 * r5)
{
	cui16 wLoc02_n = 0x0200;
	if (r4 >= (char *) (&g_t021C) + 484)
	{
		r4 = (word16 *) ((char *) &g_t021C + 483);
		if (g_ptr0082 == (char *) (&g_t021C) + 483)
			goto l0D9C;
	}
	if (r4 <= null)
	{
		r4 = null;
		if (g_ptr0082 == null)
		{
l0D9C:
			ci16 v19_n = g_w00A2 - 0x01;
			g_w00A2 = v19_n;
			if (v19_n <= 0x00)
			{
				++g_w00A4;
				g_w00A4 &= 0x03;
				++g_w00A4;
				g_w00A2 = g_w00A4;
				g_w00A6 += 0x0280;
				g_w00A6 &= 0x0380;
				++g_w00A8;
				g_w00A8 &= 0x03;
				*r5 = g_w00A6;
				*r5 |= g_w00A8;
				*r5 |= 0x8C04;
				++r5;
			}
			wchar_t r4_n = r4 - g_ptr0082;
			cui16 r4_n;
			if (r4_n <= 0x00)
			{
				wchar_t r4_n = -r4_n;
				g_ptr0082 -= r4_n & 0x3F;
				r4_n = r4_n & 0x3F | 0x40;
			}
			else
			{
				g_ptr0082 = (word16 *) ((char *) g_ptr0082 + (r4_n & 0x3F));
				r4_n = r4_n & 0x3F;
			}
			*r5 = r4_n | wLoc02_n;
			ci16 v44_n = g_w00A0 - 0x01;
			g_w00A0 = v44_n;
			cui16 * r5_n = r5 + 1;
			return r5_n;
		}
	}
	wLoc02_n = 0x4200;
	goto l0D9C;
}

// 0E06: Register int16 fn0E06(Register int16 r0)
// Called from:
//      fn053A
//      fn0856
int16 fn0E06(int16 r0)
{
	int16 r1_n = g_w0072;
	struct Eq_n * r1_n = (r1_n << 1) + 0x28F0;
	word16 wLoc02_n = 0x03;
	word16 * r1_n = &r1_n->w0002;
	word16 * r3_n = &r1_n->w0002;
	if (r1_n->w0000 >= r1_n->w0002)
		wLoc02_n = 0x04;
	fn0C90(r1_n, (byte) wLoc02_n);
	do
	{
		*r1_n -= r0;
		r3_n -= 0x02;
		*r3_n -= r0;
		++r1_n;
		r0 = -(r0 >> 1);
	} while (r0 != 0x00);
	return r0;
}

// 0E32: Register word16 fn0E32()
// Called from:
//      fn053A
//      fn0856
word16 fn0E32()
{
	g_w00AE = 0x00;
	g_w34C2 = 0x00;
	g_w34D2 = 0x00;
	*(word16 *) 62466 = *(word16 *) 62466;
	struct Eq_n * r5_n;
	do
	{
		g_w35CA = (__swab(g_w00AC + 0x01) >> 0x01) + g_w35CA & 0x0380 | 0x9C50;
		struct Eq_n * r5_n;
		fn0E98(&g_w35CC, out r5_n);
		*(word16 *) 62466 = *(word16 *) 62466;
		g_w00AE -= 0x0A;
		fn0E98(r5_n, out r5_n);
		r5_n->w0000 = 0xF700;
		r5_n->w0002 = 0x00;
		g_w34C2 = 13770;
		*(word16 *) 62466 = *(word16 *) 62466;
		g_w00AE += 33;
	} while (g_w00AE <= 0xC0);
	// Failed to bind call argument.
	// Please report this issue at https://github.com/uxmal/reko
	Eq_n stackArg0;
	stackArg0.u0 = <invalid>;
	fn13AA(stackArg0);
	__reset();
	word16 r5_n;
	return fn0E98(&r5_n->w0002, out r5_n);
}

// 0E98: Register cui16 fn0E98(Register (ptr16 Eq_n) r5, Register out (ptr16 Eq_n) r5Out)
// Called from:
//      fn0E32
cui16 fn0E98(struct Eq_n * r5, struct Eq_n & r5Out)
{
	ci16 wLoc08;
	g_w0046 = 65506;
	ci16 wLoc02_n = 0xF1;
	cui16 r0_n;
	ci16 v30_n;
	do
	{
		fn0444();
		int16 r0_n = (wLoc08 >> 1) + 0x01 + g_w0070 + g_w00AC;
		g_w00AC = r0_n;
		r0_n = r0_n & 0x1F;
		struct Eq_n * r4_n = (int16) (r0_n & 0x1F)->b2773 + g_w00AE;
		if (r4_n >= null)
		{
			word16 r2_n;
			r0_n = fn125E(r4_n, g_ptr004C, out r2_n);
			ci16 r2_n = r2_n + g_w34B4;
			if (r2_n < 0x00)
				goto l0EFE;
			r5->w0000 = r2_n | 0x4000;
			word16 r2_n;
			r0_n = fn125E(r4_n, g_ptr004A, out r2_n);
			struct Eq_n * r5_n = (struct Eq_n *) &r5->w0002;
			ci16 r2_n = r2_n + g_w34B6;
			if (r2_n >= 0x00)
			{
				r5_n->w0000 = r2_n;
				goto l0EF0;
			}
			r5_n->wFFFFFFFE = 0x00;
		}
l0EFE:
		r5->w0000 = 0x00;
		r5->w0002 = 0x00;
l0EF0:
		++g_w0046;
		v30_n = wLoc02_n - 0x01;
		++r5;
		wLoc02_n = v30_n;
	} while (v30_n > 0x00);
	r5Out = r5;
	return r0_n;
}

// 0F04: void fn0F04(Register int16 r0)
// Called from:
//      fn053A
//      fn0856
void fn0F04(int16 r0)
{
	<anonymous> * ptrLoc02;
	struct Eq_n * r5_n = fn0D3C(r0, ptrLoc02);
	g_w00AA = 0x00;
	word16 * r0_n = (g_w0074 << 1) + 0x28F0;
	struct Eq_n * sp_n = (struct Eq_n *) <invalid>;
	word16 * r4_n = fn100C(*r0_n);
	if (r4_n <= null)
		r4_n = null;
	else if (r4_n >= (char *) (&g_t021C) + 484)
		r4_n = (word16 *) ((char *) &g_t021C + 483);
	r5_n->ptr0000 = r4_n;
	g_ptr0082 = r4_n;
	r5_n->w0002 = 0x8C50;
	cui16 * r5_n = (cui16 *) (&r5_n->w0002 + 1);
	while (true)
	{
		sp_n->ptrFFFFFFFE = r4_n;
		word16 * r4_n = fn100C(*r0_n);
		word16 * v23_n = sp_n->ptrFFFFFFFE;
		sp_n->ptrFFFFFFFE = r0_n + 1;
		r4_n = v23_n;
		ci16 r1_n = r4_n - v23_n;
		wchar_t r3_n;
		if (r1_n <= 0x00)
		{
			word16 r3_n;
			fn126C(&g_t000C, null, 0x06 - r1_n, out r3_n);
			r3_n = -r3_n;
		}
		else
			fn126C(&g_t000C, null, r1_n + 0x06, out r3_n);
		ci16 r2_n;
		r0_n = sp_n->ptrFFFFFFFE;
		for (r2_n = 0x0C; r2_n > 0x00; --r2_n)
		{
			++g_w00AA;
			if (g_w00AA >= 0x03)
				g_w0F70 = 0x0ADF;
			else if (g_w00AA <= ~0x02)
				g_w0F70 = 0x0A9F;
			word16 * r4_n = (word16 *) ((char *) r4_n + g_w00AA + r3_n);
			sp_n->ptrFFFFFFFE = r4_n;
			r5_n = fn0D78(r4_n, r5_n);
			r4_n = sp_n->ptrFFFFFFFE;
		}
	}
}

word16 g_w0F70 = 0x0A9F; // 0F70
// 100C: Register int16 fn100C(Register int16 r4)
// Called from:
//      fn053A
//      fn0F04
//      fn103C
int16 fn100C(int16 r4)
{
	return (r4 * 0x03 >> 3) + 0x17;
}

// 103C: void fn103C(Register word16 r0, Register (ptr16 Eq_n) r3, Register (ptr16 Eq_n) r5, Stack (ptr16 Eq_n) wArg00)
void fn103C(word16 r0, struct Eq_n * r3, struct Eq_n * r5, struct Eq_n * wArg00)
{
	if (r5 > &g_t1F96)
		return;
	r5->w0000 = 0x9800;
	r5->w0002 = r0;
	ci16 r4_n = fn100C(r3->w0002) + fn100C(r3->w0000);
	g_w0046 = wArg00->w0000;
	int16 r4_n = (r4_n >> 1) + wArg00->w0002;
	g_w0084 = wArg00->w0004;
	r5->w0004 = r4_n;
	cup16 v32_n = r3->w0000 - r3->w0002;
	if (v32_n != 0x00)
	{
		if (v32_n <= 0x00)
			g_w0046 += ~0x15;
		else
			g_w0046 += 22;
	}
	fn0444();
	fn0242(9944, &r5->w0004 + 1);
}

// 114A: FlagGroup bool fn114A(Register (ptr16 Eq_n) r0, Register (ptr16 Eq_n) r1, Sequence out Eq_n r2_r3Out)
// Called from:
//      fn0300
//      fn0488
//      fn053A
//      fn0A0A
//      fn123A
bool fn114A(struct Eq_n * r0, struct Eq_n * r1, union Eq_n & r2_r3Out)
{
	cup16 v11_n = r1 - r0;
	Eq_n C_n = cond(v11_n) & 0x01;
	struct Eq_n * r2_n;
	if (v11_n < 0x00)
	{
		r2_n = r1;
		if (r1 == null)
			goto l119E;
		r1 = r0;
		C_n.u0 = 0x00;
	}
	else
	{
		r2_n = r0;
		if (r0 == null)
			goto l119E;
	}
	struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, C_n);
	Eq_n C_n = (r2_n & 0x8000) != 0x00;
	ui32 r2_r3_n;
	ui32 r2_r3_n;
	ui32 r2_r3_n;
	ui32 r2_r3_n;
	ui32 r2_r3_n;
	ui32 r2_r3_n;
	ui32 r2_r3_n;
	ui32 r2_r3_n;
	ui32 r2_r3_n;
	ui32 r2_r3_n;
	ui32 r2_r3_n;
	ui32 r2_r3_n;
	ui32 r2_r3_n;
	ui32 r2_r3_n;
	uint16 r3_n;
	struct Eq_n * r2_n;
	if ((r2_n & 0x8000) == 0x00)
	{
		struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, C_n);
		Eq_n C_n = (r2_n & 0x8000) != 0x00;
		r2_r3_n = (ui32) ((char *) &r2_n->ptrFFFFFFFE + 2);
		if ((r2_n & 0x8000) == 0x00)
		{
			struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, C_n);
			Eq_n C_n = (r2_n & 0x8000) != 0x00;
			r2_r3_n = (ui32) ((char *) &r2_n->ptrFFFFFFFE + 2);
			if ((r2_n & 0x8000) == 0x00)
			{
				struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, C_n);
				Eq_n C_n = (r2_n & 0x8000) != 0x00;
				r2_r3_n = (ui32) ((char *) &r2_n->ptrFFFFFFFE + 2);
				if ((r2_n & 0x8000) == 0x00)
				{
					struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, C_n);
					Eq_n C_n = (r2_n & 0x8000) != 0x00;
					r2_r3_n = (ui32) ((char *) &r2_n->ptrFFFFFFFE + 2);
					if ((r2_n & 0x8000) == 0x00)
					{
						struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, C_n);
						Eq_n C_n = (r2_n & 0x8000) != 0x00;
						r2_r3_n = (ui32) ((char *) &r2_n->ptrFFFFFFFE + 2);
						if ((r2_n & 0x8000) == 0x00)
						{
							struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, C_n);
							Eq_n C_n = (r2_n & 0x8000) != 0x00;
							r2_r3_n = (ui32) ((char *) &r2_n->ptrFFFFFFFE + 2);
							if ((r2_n & 0x8000) == 0x00)
							{
								struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, C_n);
								Eq_n C_n = (r2_n & 0x8000) != 0x00;
								r2_r3_n = (ui32) ((char *) &r2_n->ptrFFFFFFFE + 2);
								if ((r2_n & 0x8000) == 0x00)
								{
									struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, C_n);
									Eq_n C_n = (r2_n & 0x8000) != 0x00;
									r2_r3_n = (ui32) ((char *) &r2_n->ptrFFFFFFFE + 2);
									if ((r2_n & 0x8000) == 0x00)
									{
										struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, C_n);
										Eq_n C_n = (r2_n & 0x8000) != 0x00;
										r2_r3_n = (ui32) ((char *) &r2_n->ptrFFFFFFFE + 2);
										if ((r2_n & 0x8000) == 0x00)
										{
											struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, C_n);
											Eq_n C_n = (r2_n & 0x8000) != 0x00;
											r2_r3_n = (ui32) ((char *) &r2_n->ptrFFFFFFFE + 2);
											if ((r2_n & 0x8000) == 0x00)
											{
												struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, C_n);
												Eq_n C_n = (r2_n & 0x8000) != 0x00;
												r2_r3_n = (ui32) ((char *) &r2_n->ptrFFFFFFFE + 2);
												if ((r2_n & 0x8000) == 0x00)
												{
													struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, C_n);
													Eq_n C_n = (r2_n & 0x8000) != 0x00;
													r2_r3_n = (ui32) ((char *) &r2_n->ptrFFFFFFFE + 2);
													if ((r2_n & 0x8000) == 0x00)
													{
														struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, C_n);
														Eq_n C_n = (r2_n & 0x8000) != 0x00;
														r2_r3_n = (ui32) ((char *) &r2_n->ptrFFFFFFFE + 2);
														if ((r2_n & 0x8000) == 0x00)
														{
															r2_r3_n = (ui32) ((char *) &__rcl<word16,byte>(r2_n, 0x01, C_n)->ptrFFFFFFFE + 2);
															if ((r2_n & 0x8000) == 0x00)
															{
																C_n.u0 = 0x00;
l119E:
																r2_r3Out.u0 = <invalid>;
																return C_n != 0x00;
															}
															goto l122A;
														}
														goto l1220;
													}
													goto l1216;
												}
												goto l120C;
											}
											goto l1202;
										}
										goto l11F8;
									}
									goto l11EE;
								}
								goto l11E4;
							}
							goto l11DA;
						}
						goto l11D0;
					}
					goto l11C6;
				}
				goto l11BC;
			}
			goto l11B2;
		}
	}
	else
	{
		r3_n = r1 << 1;
		r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01);
		r2_r3_n = (ui32) r3_n;
		if ((r2_n & 0x8000) == 0x00)
			goto l11AC;
	}
	uint32 r2_r3_n = r2_r3_n + (uint32) r1;
	r3_n = (word16) r2_r3_n;
	r2_n = SLICE(r2_r3_n, word16, 16);
l11AC:
	uint16 r3_n = r3_n << 1;
	struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01);
	r2_r3_n = (ui32) r3_n;
	if ((r2_n & 0x8000) == 0x00)
	{
l11B6:
		uint16 r3_n = r3_n << 1;
		struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01);
		r2_r3_n = (ui32) r3_n;
		if ((r2_n & 0x8000) == 0x00)
		{
l11C0:
			uint16 r3_n = r3_n << 1;
			struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01);
			r2_r3_n = (ui32) r3_n;
			if ((r2_n & 0x8000) == 0x00)
			{
l11CA:
				uint16 r3_n = r3_n << 1;
				struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01);
				r2_r3_n = (ui32) r3_n;
				if ((r2_n & 0x8000) == 0x00)
				{
l11D4:
					uint16 r3_n = r3_n << 1;
					struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01);
					r2_r3_n = (ui32) r3_n;
					if ((r2_n & 0x8000) == 0x00)
					{
l11DE:
						uint16 r3_n = r3_n << 1;
						struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01);
						r2_r3_n = (ui32) r3_n;
						if ((r2_n & 0x8000) == 0x00)
						{
l11E8:
							uint16 r3_n = r3_n << 1;
							struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01);
							r2_r3_n = (ui32) r3_n;
							if ((r2_n & 0x8000) == 0x00)
							{
l11F2:
								uint16 r3_n = r3_n << 1;
								struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01);
								r2_r3_n = (ui32) r3_n;
								if ((r2_n & 0x8000) == 0x00)
								{
l11FC:
									uint16 r3_n = r3_n << 1;
									struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01);
									r2_r3_n = (ui32) r3_n;
									if ((r2_n & 0x8000) == 0x00)
									{
l1206:
										uint16 r3_n = r3_n << 1;
										struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01);
										r2_r3_n = (ui32) r3_n;
										if ((r2_n & 0x8000) == 0x00)
										{
l1210:
											uint16 r3_n = r3_n << 1;
											struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01);
											r2_r3_n = (ui32) r3_n;
											if ((r2_n & 0x8000) == 0x00)
											{
l121A:
												uint16 r3_n = r3_n << 1;
												struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01);
												r2_r3_n = (ui32) r3_n;
												if ((r2_n & 0x8000) == 0x00)
												{
l1224:
													uint16 r3_n = r3_n << 1;
													struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01);
													r2_r3_n = (ui32) r3_n;
													if ((r2_n & 0x8000) == 0x00)
													{
l122E:
														uint16 r3_n = r3_n << 1;
														struct Eq_n * r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01);
														Eq_n C_n = (r2_n & 0x8000) != 0x00;
														if ((r2_n & 0x8000) != 0x00)
															C_n = cond(SLICE(r3_n + (uint32) r1, word16, 16)) & 0x01;
														r2_r3Out.u0 = <invalid>;
														return C_n != 0x00;
													}
l122A:
													uint32 r2_r3_n = r2_r3_n + (uint32) r1;
													r3_n = (word16) r2_r3_n;
													r2_n = SLICE(r2_r3_n, word16, 16);
													goto l122E;
												}
l1220:
												uint32 r2_r3_n = r2_r3_n + (uint32) r1;
												r3_n = (word16) r2_r3_n;
												r2_n = SLICE(r2_r3_n, word16, 16);
												goto l1224;
											}
l1216:
											uint32 r2_r3_n = r2_r3_n + (uint32) r1;
											r3_n = (word16) r2_r3_n;
											r2_n = SLICE(r2_r3_n, word16, 16);
											goto l121A;
										}
l120C:
										uint32 r2_r3_n = r2_r3_n + (uint32) r1;
										r3_n = (word16) r2_r3_n;
										r2_n = SLICE(r2_r3_n, word16, 16);
										goto l1210;
									}
l1202:
									uint32 r2_r3_n = r2_r3_n + (uint32) r1;
									r3_n = (word16) r2_r3_n;
									r2_n = SLICE(r2_r3_n, word16, 16);
									goto l1206;
								}
l11F8:
								uint32 r2_r3_n = r2_r3_n + (uint32) r1;
								r3_n = (word16) r2_r3_n;
								r2_n = SLICE(r2_r3_n, word16, 16);
								goto l11FC;
							}
l11EE:
							uint32 r2_r3_n = r2_r3_n + (uint32) r1;
							r3_n = (word16) r2_r3_n;
							r2_n = SLICE(r2_r3_n, word16, 16);
							goto l11F2;
						}
l11E4:
						uint32 r2_r3_n = r2_r3_n + (uint32) r1;
						r3_n = (word16) r2_r3_n;
						r2_n = SLICE(r2_r3_n, word16, 16);
						goto l11E8;
					}
l11DA:
					uint32 r2_r3_n = r2_r3_n + (uint32) r1;
					r3_n = (word16) r2_r3_n;
					r2_n = SLICE(r2_r3_n, word16, 16);
					goto l11DE;
				}
l11D0:
				uint32 r2_r3_n = r2_r3_n + (uint32) r1;
				r3_n = (word16) r2_r3_n;
				r2_n = SLICE(r2_r3_n, word16, 16);
				goto l11D4;
			}
l11C6:
			uint32 r2_r3_n = r2_r3_n + (uint32) r1;
			r3_n = (word16) r2_r3_n;
			r2_n = SLICE(r2_r3_n, word16, 16);
			goto l11CA;
		}
l11BC:
		uint32 r2_r3_n = r2_r3_n + (uint32) r1;
		r3_n = (word16) r2_r3_n;
		r2_n = SLICE(r2_r3_n, word16, 16);
		goto l11C0;
	}
l11B2:
	uint32 r2_r3_n = r2_r3_n + (uint32) r1;
	r3_n = (word16) r2_r3_n;
	r2_n = SLICE(r2_r3_n, word16, 16);
	goto l11B6;
}

// 123A: Register (ptr16 Eq_n) fn123A(Register (ptr16 Eq_n) r0, Register (ptr16 Eq_n) r1, Register out (ptr16 Eq_n) r2Out, Register out (ptr16 Eq_n) r3Out)
// Called from:
//      fn053A
//      fn125E
struct Eq_n * fn123A(struct Eq_n * r0, struct Eq_n * r1, struct Eq_n & r2Out, struct Eq_n & r3Out)
{
	if (r0 <= null)
	{
		r0 = -r0;
		if (r1 <= null)
		{
			r1 = -r1;
l1246:
			ui32 r2_r3_n;
			fn114A(r0, r1, out r2_r3_n);
			struct Eq_n * r2_n = (struct Eq_n *) <invalid>;
			r2Out = r2_n;
			struct Eq_n * r3_n = (struct Eq_n *) <invalid>;
			r3Out = r3_n;
			return r0;
		}
	}
	else
	{
		if (r1 > null)
			goto l1246;
		r1 = -r1;
	}
	struct Eq_n * r2_n = (struct Eq_n *) <invalid>;
	ui32 r2_r3_n;
	r2Out = -(r2_n + fn114A(r0, r1, out r2_r3_n));
	struct Eq_n * r3_n = (struct Eq_n *) <invalid>;
	r3Out = -r3_n;
	return r0;
}

// 125E: Register (ptr16 Eq_n) fn125E(Register (ptr16 Eq_n) r0, Register (ptr16 Eq_n) r1, Register out (ptr16 Eq_n) r2Out)
// Called from:
//      fn02C8
//      fn0300
//      fn0E98
struct Eq_n * fn125E(struct Eq_n * r0, struct Eq_n * r1, struct Eq_n & r2Out)
{
	cui16 r3_n;
	struct Eq_n * r2_n;
	struct Eq_n * r0_n = fn123A(r0, r1, out r2_n, out r3_n);
	r2Out = __rcl<word16,byte>(__rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01), 0x01, cond(r3_n << 2) & 0x01);
	return r0_n;
}

// 126C: Register (ptr16 Eq_n) fn126C(Register (ptr16 Eq_n) r0, Register (ptr16 Eq_n) r2, Register (ptr16 Eq_n) r3, Register out Eq_n r3Out)
// Called from:
//      fn0300
//      fn0488
//      fn053A
//      fn0A0A
//      fn0F04
//      fn1578
//      fn15F2
struct Eq_n * fn126C(struct Eq_n * r0, struct Eq_n * r2, struct Eq_n * r3, union Eq_n & r3Out)
{
	struct Eq_n * r2_n = __rcl<word16,byte>(r2, 0x01, cond(r3 << 1) & 0x01) - r0;
	ui16 r3_n;
	struct Eq_n * r2_n;
	ui16 r3_n;
	struct Eq_n * r2_n;
	if (r2_n <= null)
	{
		r3_n = r3 << 2;
		r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3 << 2) & 0x01) + r0;
		if (r2_n <= null)
		{
l127C:
			r3_n = r3_n << 1;
			r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01) + r0;
			ui16 r3_n;
			struct Eq_n * r2_n;
			if (r2_n <= null)
			{
l1284:
				r3_n = r3_n << 1;
				r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01) + r0;
				ui16 r3_n;
				struct Eq_n * r2_n;
				if (r2_n <= null)
				{
l128C:
					r3_n = r3_n << 1;
					r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01) + r0;
					ui16 r3_n;
					struct Eq_n * r2_n;
					if (r2_n <= null)
					{
l1294:
						r3_n = r3_n << 1;
						r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01) + r0;
						ui16 r3_n;
						struct Eq_n * r2_n;
						if (r2_n <= null)
						{
l129C:
							r3_n = r3_n << 1;
							r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01) + r0;
							ui16 r3_n;
							struct Eq_n * r2_n;
							if (r2_n <= null)
							{
l12A4:
								r3_n = r3_n << 1;
								r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01) + r0;
								ui16 r3_n;
								struct Eq_n * r2_n;
								if (r2_n <= null)
								{
l12AC:
									r3_n = r3_n << 1;
									r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01) + r0;
									ui16 r3_n;
									struct Eq_n * r2_n;
									if (r2_n <= null)
									{
l12B4:
										r3_n = r3_n << 1;
										r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01) + r0;
										ui16 r3_n;
										struct Eq_n * r2_n;
										if (r2_n <= null)
										{
l12BC:
											r3_n = r3_n << 1;
											r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01) + r0;
											ui16 r3_n;
											struct Eq_n * r2_n;
											if (r2_n <= null)
											{
l12C4:
												r3_n = r3_n << 1;
												r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01) + r0;
												ui16 r3_n;
												struct Eq_n * r2_n;
												if (r2_n <= null)
												{
l12CC:
													r3_n = r3_n << 1;
													r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01) + r0;
													ui16 r3_n;
													struct Eq_n * r2_n;
													if (r2_n <= null)
													{
l12D4:
														r3_n = r3_n << 1;
														r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01) + r0;
														ui16 r3_n;
														struct Eq_n * r2_n;
														if (r2_n <= null)
														{
l12DC:
															r3_n = r3_n << 1;
															r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01) + r0;
															Eq_n r3_n;
															struct Eq_n * r2_n;
															if (r2_n <= null)
															{
l12E4:
																r3_n = r3_n << 1;
																r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n) & 0x01) + r0;
																if (r2_n <= null)
																{
l12EC:
																	r3Out = r3_n;
																	return r2_n + r0;
																}
l1386:
																Eq_n r3_n = (word16) r3_n + 1;
																r3Out = r3_n;
																return r2_n;
															}
l137C:
															ui16 r3_n = r3_n + 0x01;
															r3_n = r3_n << 1;
															r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01) - r0;
															if (r2_n < null)
																goto l12EC;
															goto l1386;
														}
l1372:
														ui16 r3_n = r3_n + 0x01;
														r3_n = r3_n << 1;
														r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01) - r0;
														if (r2_n < null)
															goto l12E4;
														goto l137C;
													}
l1368:
													ui16 r3_n = r3_n + 0x01;
													r3_n = r3_n << 1;
													r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01) - r0;
													if (r2_n < null)
														goto l12DC;
													goto l1372;
												}
l135E:
												ui16 r3_n = r3_n + 0x01;
												r3_n = r3_n << 1;
												r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01) - r0;
												if (r2_n < null)
													goto l12D4;
												goto l1368;
											}
l1354:
											ui16 r3_n = r3_n + 0x01;
											r3_n = r3_n << 1;
											r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01) - r0;
											if (r2_n < null)
												goto l12CC;
											goto l135E;
										}
l134A:
										ui16 r3_n = r3_n + 0x01;
										r3_n = r3_n << 1;
										r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01) - r0;
										if (r2_n < null)
											goto l12C4;
										goto l1354;
									}
l1340:
									ui16 r3_n = r3_n + 0x01;
									r3_n = r3_n << 1;
									r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01) - r0;
									if (r2_n < null)
										goto l12BC;
									goto l134A;
								}
l1336:
								ui16 r3_n = r3_n + 0x01;
								r3_n = r3_n << 1;
								r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01) - r0;
								if (r2_n < null)
									goto l12B4;
								goto l1340;
							}
l132C:
							ui16 r3_n = r3_n + 0x01;
							r3_n = r3_n << 1;
							r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01) - r0;
							if (r2_n < null)
								goto l12AC;
							goto l1336;
						}
l1322:
						ui16 r3_n = r3_n + 0x01;
						r3_n = r3_n << 1;
						r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01) - r0;
						if (r2_n < null)
							goto l12A4;
						goto l132C;
					}
l1318:
					ui16 r3_n = r3_n + 0x01;
					r3_n = r3_n << 1;
					r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01) - r0;
					if (r2_n < null)
						goto l129C;
					goto l1322;
				}
l130E:
				ui16 r3_n = r3_n + 0x01;
				r3_n = r3_n << 1;
				r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01) - r0;
				if (r2_n < null)
					goto l1294;
				goto l1318;
			}
l1304:
			ui16 r3_n = r3_n + 0x01;
			r3_n = r3_n << 1;
			r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01) - r0;
			if (r2_n < null)
				goto l128C;
			goto l130E;
		}
	}
	else
	{
		ui16 r3_n = (r3 << 1) + 0x01;
		r3_n = r3_n << 1;
		r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01) - r0;
		if (r2_n < null)
			goto l127C;
	}
	ui16 r3_n = r3_n + 0x01;
	r3_n = r3_n << 1;
	r2_n = __rcl<word16,byte>(r2_n, 0x01, cond(r3_n << 1) & 0x01) - r0;
	if (r2_n < null)
		goto l1284;
	goto l1304;
}

// 13AA: void fn13AA(Stack Eq_n ptrArg00)
// Called from:
//      fn0856
//      fn0B06
//      fn0E32
//      fn34E0
void fn13AA(Eq_n ptrArg00)
{
	g_w25C0 = 0xF700;
	ci16 r0_n = *ptrArg00.u1;
	cu16 r1_n = g_w0070;
	ci16 r0_n = r0_n;
	if (r0_n <= 0x00)
		r0_n = -r0_n;
	do
	{
		r1_n += 0x3C;
		--r0_n;
	} while (r0_n > 0x00);
	do
	{
		__wait();
		fn1578();
	} while (r1_n > g_w0070);
	if (r0_n < 0x00)
		return;
	__reset();
	word16 * r0_n = g_a182A;
	ci16 r1_n;
	for (r1_n = 500; r1_n > 0x00; --r1_n)
	{
		*r0_n = 0xF700;
		word16 * r0_n = r0_n + 1;
		*r0_n = 0x00;
		r0_n = r0_n + 1;
	}
	word16 * r0_n = g_a34E0;
	ci16 r1_n;
	for (r1_n = 711; r1_n > 0x00; --r1_n)
	{
		*r0_n = 0xF700;
		word16 * r0_n = r0_n + 1;
		*r0_n = 0x00;
		r0_n = r0_n + 1;
	}
	*(word16 *) ~0x01 = 0x00;
	word16 * r0_n = g_a013E;
	while (true)
	{
		word16 * v24_n = *r0_n;
		word16 * r0_n = r0_n + 1;
		if (v24_n == null)
			break;
		*v24_n = *r0_n;
		r0_n = r0_n + 1;
	}
	while (true)
		fn0128();
}

// 1578: void fn1578()
// Called from:
//      fn053A
//      fn0856
//      fn13AA
void fn1578()
{
	struct Eq_n * r4_n = null;
	while (true)
	{
		struct Eq_n * r5_n = (struct Eq_n *) r4_n[2886];
		if (r5_n == null)
			break;
		*((word16) r4_n[0x0B4B] + 18) = (struct Eq_n) ((char *) r5_n + 0x0A);
		struct Eq_n * r3_n = (struct Eq_n *) *r5_n->ptrFFFC;
		struct Eq_n * r0_n = r5_n->ptrFFFE;
		if (r0_n != null)
		{
			if (r3_n <= null)
				r3_n = -r3_n;
			fn126C(r0_n, null, r3_n, out r3_n);
			if (*r5_n->ptrFFFC <= null)
				r3_n = -r3_n;
		}
		fn1674(r3_n, (word16) r4_n[0x0B4B] + 0x0A);
		++r4_n;
	}
	if (g_ptr0060 != g_ptr0062)
	{
		struct Eq_n * r0_n = g_ptr0060;
		g_ptr0062 = r0_n;
		fn1674(r0_n, &g_t25B8);
	}
}

// 15F2: Register (ptr16 Eq_n) fn15F2(Register (ptr16 Eq_n) r0, Register (ptr16 Eq_n) r1)
// Called from:
//      fn1674
struct Eq_n * fn15F2(struct Eq_n * r0, struct Eq_n * r1)
{
	r1->b0000 = 0x20;
	ptr16 ptrLoc02_n;
	if (r0 < &g_t2710)
	{
		r1->b0001 = 0x20;
		ptrLoc02_n = null;
	}
	else
	{
		r1->b0001 = 0x30;
		ptrLoc02_n = &g_t163C;
		for (; r0 >= &g_t2710; r0 -= 10000)
			++r1->b0001;
	}
	struct Eq_n * r2_n;
	byte * r3_n;
	if (r0 >= &g_ptr0064)
	{
		cui16 r3_n;
		r2_n = fn126C(&g_ptr0064, null, r0, out r3_n);
		r3_n = (r3_n << 1) + 10260;
	}
	else
	{
		r2_n = r0;
		r3_n = &g_b2814;
	}
	byte * r3_n;
	word16 r3_n;
	byte * r3_n;
	word16 r3_n;
	return fn1658(fn1658(fn1658(fn1658(&r1->b0001 + 1, r3_n, ptrLoc02_n, out r3_n), r3_n, ptrLoc02_n, out r3_n), (r2_n << 1) + 10260, ptrLoc02_n, out r3_n), r3_n, &g_t1630, out r3_n);
}

<unknown> g_t1630 = // 1630;
<unknown> g_t163C = // 163C;
// 1658: Register (ptr16 Eq_n) fn1658(Register (ptr16 Eq_n) r1, Register (ptr16 byte) r3, Stack ptr16 wArg02, Register out ptr16 r3Out)
// Called from:
//      fn15F2
struct Eq_n * fn1658(struct Eq_n * r1, byte * r3, ptr16 wArg02, ptr16 & r3Out)
{
	if (wArg02 != null || *r3 != 0x30)
	{
		r1->b0000 = *r3;
		return;
	}
	else
	{
		r1->b0000 = 0x20;
		return;
	}
}

// 1674: void fn1674(Register (ptr16 Eq_n) r0, Register (ptr16 Eq_n) r1)
// Called from:
//      fn1578
void fn1674(struct Eq_n * r0, struct Eq_n * r1)
{
	if (r0 > null)
		fn15F2(r0, r1);
	else
	{
		byte * r0_n = fn15F2(-r0, r1);
		do
			--r0_n;
		while (*r0_n != 0x20);
		*r0_n = 0x2D;
	}
}

struct Eq_n * g_a168C[] = // 168C
	{
	};
struct Eq_n * g_a1696[] = // 1696
	{
	};
word16 g_a182A[] = // 182A
	{
	};
word16 g_w182C = 0x00; // 182C
word16 g_w182E = 0x00; // 182E
Eq_n g_t1F96 = // 1F96
	{
		0x00,
		0x00,
		0,
	};
word16 g_w25AA = 0x00; // 25AA
Eq_n g_t25B8 = // 25B8
	{
		0x20,
		0x20,
	};
word16 g_w25C0 = 0xF700; // 25C0
struct Eq_n g_t2710 = // 2710
	{
		&g_wFFFFF011,
	};
cui16 g_w27A8 = 0x00; // 27A8
Eq_n g_a27B0[] = // 27B0
	{
	};
byte g_b2814 = 0x30; // 2814
ci16 g_w28F0 = 0x030C; // 28F0
word16 g_a28F2[] = // 28F2
	{
	};
Eq_n g_a28F8[] = // 28F8
	{
	};
struct Eq_n g_t2904 = // 2904
	{
		&g_w015E,
	};
struct Eq_n * g_a31DC[] = // 31DC
	{
	};
word16 g_w34B4 = 0x00; // 34B4
word16 g_w34B6 = 0x00; // 34B6
word16 g_w34C2 = 0x00; // 34C2
word16 g_w34CA = 0x00; // 34CA
word16 g_w34D2 = 0x00; // 34D2
word16 g_w34DA = 0x00; // 34DA
// 34E0: void fn34E0()
void fn34E0()
{
	__reset();
	__reset();
	__reset();
	__reset();
	__reset();
	*(word16 *) ~0x01 = 0x00;
	g_w0004 = 13638;
	g_w0006 = 0x00;
	*(word16 *) ~0x0471 = 0x00;
	fn355A();
	*(word16 *) ~0x0471 = 131;
	fn355A();
	*(word16 *) ~0x0471 = 131;
	fn355A();
	*(word16 *) ~0x0471 = 0x4B;
	fn355A();
	*(word16 *) ~0x0471 = 0x2F;
	fn355A();
	*(word16 *) ~0x0471 = 0x46;
	fn355A();
	*(word16 *) ~0x0471 = 0x0D;
	fn355A();
	*(word16 *) ~0x99 = 0x40;
	*(word16 *) 0xF400 = 13666;
	fn13AA(*(union Eq_n *) 0x3FFC);
}

// 355A: void fn355A()
// Called from:
//      fn34E0
void fn355A()
{
	while (*(ci8 *) 64396 > 0x00)
		;
}

cui16 g_w35CA = 17221; // 35CA
word16 g_w35CC = 0x5541; // 35CC
word16 g_w35CE = 17747; // 35CE
word16 g_w35D0 = 0x4F20; // 35D0
cui16 g_a35D2[] = // 35D2
	{
	};
