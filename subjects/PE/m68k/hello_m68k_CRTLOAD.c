// hello_m68k_CRTLOAD.c
// Generated by decompiling hello_m68k.exe
// using Reko decompiler version 0.12.5.0.

#include "hello_m68k.h"

// 00001498: void fn00001498()
void fn00001498()
{
}

// 0000149C: void fn0000149C()
void fn0000149C()
{
	__syscall<word16>(43424);
	if (false)
		*(word16 *) 0x0AF0 = 0x1A;
	else
	{
		__syscall<word16>(43118);
		__syscall<word16>(43262);
		__syscall<word16>(43282);
		__syscall<word16>(0xA930);
		__syscall<word16>(0xA9CC);
		__syscall<word16>(43387);
		__syscall<word16>(0xA850);
		__syscall<word16>(43398);
	}
	__syscall<word16>(~0x560B);
}

// 000014E8: void fn000014E8(Register word32 a3, Register word32 a5, Stack word16 wArg00, Stack int32 dwArg02)
void fn000014E8(word32 a3, word32 a5, word16 wArg00, int32 dwArg02)
{
	word16 wLoc12;
	0x00 = 0x00;
	__syscall<word16>(43424);
	struct Eq_n ** dwLoc12_n = SEQ(wLoc12, 0x4441);
	word32 dwLoc02_n = SEQ((word16) a3, wArg00);
	if (dwLoc12_n == null)
	{
		*(word16 *) 0x0AF0 = ~0x025C;
		__syscall<word16>(~0x560B);
		goto l0000159A;
	}
	else
	{
		struct Eq_n * a3_n = (struct Eq_n *) *dwLoc12_n;
		uint32 d0_n = a3_n->dw0000;
		word16 * a2_n = a5 - d0_n;
		word16 * a0_n = a2_n;
		uint32 d0_n = d0_n >> 0x01;
		if (d0_n >> 0x01 != 0x00)
		{
			do
			{
				*a0_n = 0x00;
				++a0_n;
				--d0_n;
			} while (d0_n != 0x00);
		}
		word32 a2_n = a2_n - a3_n->dw0004;
		__syscall<word16>(0xA02E);
		uint32 d0_n = a3_n->dw0008;
		word16 * a2_n = a2_n - d0_n;
		word16 * a0_n = a2_n;
		uint32 d0_n = d0_n >> 0x01;
		if (d0_n >> 0x01 != 0x00)
		{
			do
			{
				*a0_n = 0x00;
				++a0_n;
				--d0_n;
			} while (d0_n != 0x00);
		}
		union Eq_n * a2_n = a2_n - a3_n->dw000C;
		struct Eq_n * a3_n = (struct Eq_n *) ((char *) &(&a3_n->dw000C + 1)->dw0000 + a3_n->dw0004 + a3_n->dw000C);
		__syscall<word16>(0xA02E);
l00001556:
		Eq_n v18_n = a3_n->b0000;
		++a3_n;
		uint32 d0_n = (uint32) v18_n;
		ui32 d0_n;
		if (v18_n <= 0x00)
		{
			if (v18_n >= 0x00)
			{
				uint8 v22_n = a3_n->b0000;
				if (v22_n != 0x00)
				{
					word24 v29_n = SLICE(__swap(SEQ(SLICE((uint32) ((word16) v22_n << 0x08), word24, 8), a3_n[1])), word24, 8);
					a3_n += 4;
					d0_n = SEQ(SLICE(SEQ(SLICE(v29_n, word16, 8), (word16) SEQ(v29_n, a3_n[2]) << 0x08), word24, 8), a3_n[3]) * 0x02;
l00001578:
					a2_n = (union Eq_n *) ((char *) &a2_n->u0 + d0_n);
					if (dwArg02 != 0x00 && a2_n->u0 >= 0x00)
						a2_n->u0 = (a2_n->u0 + dwArg02)->t0002.u0;
					else
						a2_n->u0 = (int32) (a2_n->u0 + dwLoc02_n);
					goto l00001556;
				}
l0000159A:
				__syscall<word16>(0xA9A3);
				return;
			}
			++a3_n;
			d0_n = SEQ(SLICE((uint32) ((word16) v18_n << 0x08), word24, 8), a3_n->b0000);
		}
		d0_n = SEQ(SLICE(d0_n, word16, 16), (word16) d0_n * 0x02);
		goto l00001578;
	}
}

