// main.c
// Generated by decompiling main.exe
// using Reko decompiler version 0.6.2.0.

#include "main.h"

// 00401000: void fn00401000()
void fn00401000()
{
	return;
}

// 00401010: Register word32 fn00401010(Stack word32 dwArg04)
word32 fn00401010(word32 dwArg04)
{
	word32 esp_13;
	word32 ebp_14;
	word32 esi_15;
	byte SCZO_16;
	word32 ecx_17;
	word32 eax_18;
	_acrt_iob_func();
	fn00401000();
	word32 esp_38;
	word32 ebp_39;
	word32 esi_40;
	byte SCZO_41;
	word32 ecx_42;
	word32 eax_43;
	_stdio_common_vfprintf();
	return 0x01;
}

// 00401040: Register word32 fn00401040()
word32 fn00401040()
{
	word32 esp_10;
	word32 ebp_11;
	word32 ecx_12;
	word32 eax_13;
	byte SCZO_14;
	byte SZO_15;
	byte C_16;
	driver.dll!Ordinal_1();
	word32 esp_20;
	word32 ebp_21;
	word32 ecx_22;
	word32 eax_23;
	byte SCZO_24;
	byte SZO_25;
	byte C_26;
	driver.dll!Ordinal_2();
	word32 esp_30;
	word32 ebp_31;
	word32 ecx_32;
	word32 eax_33;
	byte SCZO_34;
	byte SZO_35;
	byte C_36;
	driver.dll!Ordinal_4();
	word32 esp_40;
	word32 ebp_41;
	word32 ecx_42;
	word32 eax_43;
	byte SCZO_44;
	byte SZO_45;
	byte C_46;
	driver.dll!Ordinal_3();
	fn00401010(0x00402118);
	return 0x00;
}

// 004012C9: Register Eq_62 Win32CrtStartup()
Eq_62 Win32CrtStartup()
{
	ptr32 ecx_2 = fn0040165E();
	struct Eq_67 * ebp_12 = fn00401980(ecx_2, ebx, esi, edi, dwLoc0C, 0x00402508, 0x14);
	*(fp - 0x0C) = 0x01;
	word32 * esp_19 = fp - 0x08;
	if (fn0040146F(edx, dwArg00) == 0x00)
	{
l00401172:
		esp_19 = esp_19 - 0x04;
		*esp_19 = 0x07;
		fn0040176D(0x14, dwArg00);
	}
	*(ebp_12 - 0x19) = 0x00;
	*(ebp_12 - 0x04) = *(ebp_12 - 0x04) & 0x00;
	*(ebp_12 - 0x24) = fn0040143A(0x14);
	ebx = DPB(ebx, 0x00, 0);
	word32 eax_37 = globals->dw403334;
	if (eax_37 != 0x01)
	{
		Eq_62 eax_191;
		if (eax_37 == 0x00)
		{
			globals->dw403334 = 0x01;
			PVFV ** esp_280 = esp_19 - 0x04;
			*esp_280 = (PVFV **) &globals->t4020F0;
			*(esp_280 - 0x04) = 0x004020E4;
			if (_initterm_e(*(esp_280 - 0x04), *esp_280) != 0x00)
			{
				*(ebp_12 - 0x04) = ~0x01;
				eax_191.u0 = 0xFF;
				goto l004012C3;
			}
			*esp_280 = (PVFV **) &globals->t4020E0;
			*(esp_280 - 0x04) = 0x004020D8;
			_initterm(*(esp_280 - 0x04), *esp_280);
			globals->dw403334 = 0x02;
		}
		else
		{
			*(ebp_12 - 0x19) = 0x01;
			ebx = DPB(ebx, 0x01, 0);
		}
		ptr32 * esp_73 = esp_19 - 0x04;
		*esp_73 = *(ebp_12 - 0x24);
		fn004015C9(bArg00);
		ptr32 ecx_249 = *esp_73;
		fn00401761();
		ptr32 esp_251 = esp_73 + 0x01;
		ptr32 edi_246 = 0x00;
		if (globals->dw403384 != 0x00)
		{
			*esp_73 = 0x00403384;
			<anonymous> ** esi_245;
			word32 eax_247 = fn0040153F(ecx_249, ebx, 0x00403384, 0x00, out ebx, out esi_245, out edi_246);
			byte al_248 = (byte) eax_247;
			ecx_249 = *esp_73;
			esp_251 = esp_73 + 0x01;
			if (al_248 != 0x00)
			{
				*esp_73 = edi_246;
				*(esp_73 - 0x04) = 0x02;
				*(esp_73 - 0x08) = edi_246;
				<anonymous> * esi_261 = *esi_245;
				fn00401972();
				byte al_266;
				byte SZO_267;
				byte C_268;
				byte Z_269;
				byte bl_270;
				word32 ebp_271;
				word32 eax_272;
				byte SCZO_273;
				byte cl_274;
				word32 esi_275;
				word32 edx_278;
				esi_261();
			}
		}
		word32 eax_93 = fn00401767();
		if (globals->ptr403380 != edi_246)
		{
			ptr32 * esp_209 = esp_251 - 0x04;
			*esp_209 = 0x00403380;
			word32 ebx_211;
			ptr32 * esi_212;
			word32 edi_213;
			word32 eax_214 = fn0040153F(ecx_249, ebx, 0x00403380, edi_246, out ebx_211, out esi_212, out edi_213);
			byte al_215 = (byte) eax_214;
			if (al_215 != 0x00)
			{
				*esp_209 = *esi_212;
				word32 esp_224;
				word32 ecx_225;
				byte al_226;
				byte SZO_227;
				byte C_228;
				byte Z_229;
				byte bl_230;
				word32 ebp_231;
				word32 eax_232;
				byte SCZO_233;
				byte cl_234;
				word32 esi_235;
				word32 edi_236;
				word32 ebx_237;
				word32 edx_238;
				register_thread_local_exe_atexit_callback();
			}
		}
		word32 esp_98;
		word32 ecx_99;
		byte al_100;
		byte SZO_101;
		byte C_102;
		byte Z_103;
		byte bl_104;
		word32 ebp_105;
		word32 eax_106;
		byte SCZO_107;
		byte cl_108;
		word32 esi_109;
		word32 edi_110;
		word32 ebx_111;
		word32 edx_112;
		_p___argv();
		word32 esp_115;
		word32 ecx_116;
		byte al_117;
		byte SZO_118;
		byte C_119;
		byte Z_120;
		byte bl_121;
		word32 ebp_122;
		word32 eax_123;
		byte SCZO_124;
		byte cl_125;
		word32 esi_126;
		word32 edi_127;
		word32 ebx_128;
		word32 edx_129;
		_p___argc();
		struct Eq_380 * esp_132;
		word32 ecx_133;
		byte al_134;
		byte SZO_135;
		byte C_136;
		byte Z_137;
		byte bl_138;
		Eq_62 eax_140;
		byte SCZO_141;
		byte cl_142;
		word32 * esi_143;
		word32 * edi_144;
		word32 ebx_145;
		word32 edx_146;
		get_initial_narrow_environment();
		esp_132->tFFFFFFF8 = eax_140;
		esp_132->dwFFFFFFF4 = *edi_144;
		esp_132->dwFFFFFFF0 = *esi_143;
		Eq_62 eax_156 = fn00401040();
		ptr32 esp_157 = &esp_132->tFFFFFFF8;
		Eq_62 esi_159 = eax_156;
		byte al_160 = fn0040188B();
		if (al_160 == 0x00)
		{
			esp_132->tFFFFFFF8 = eax_156;
			exit(esp_132->tFFFFFFF8);
		}
		if (bl_138 == 0x00)
		{
			word32 esp_193;
			word32 ecx_194;
			byte al_195;
			byte SZO_196;
			byte C_197;
			byte Z_198;
			byte bl_199;
			word32 eax_201;
			byte SCZO_202;
			byte cl_203;
			word32 edi_205;
			word32 ebx_206;
			word32 edx_207;
			cexit();
			esp_157 = esp_193 + ~0x03;
		}
		word32 * esp_181 = esp_157 - 0x04;
		*esp_181 = 0x00;
		*(esp_181 - 0x04) = 0x01;
		fn004015E6(dwArg00, bArg04);
		*(ebp_12 - 0x04) = ~0x01;
		eax_191 = esi_159;
l004012C3:
		word32 ebp_62;
		word32 esi_63;
		word32 edi_64;
		fn004019C6(ebp_12, 0x14, dwArg00, dwArg04, dwArg08, dwArg0C, out ebp_62, out esi_63, out edi_64);
		return eax_191;
	}
	goto l00401172;
}

// 004012D3: void fn004012D3(Stack word32 dwArg04)
void fn004012D3(word32 dwArg04)
{
	SetUnhandledExceptionFilter(null);
	UnhandledExceptionFilter(dwArg04);
	TerminateProcess(GetCurrentProcess(), 0xC0000409);
	return;
}

// 004013F6: Register (ptr Eq_542) fn004013F6(Stack ui32 dwArg04, Stack word32 dwArg08)
Eq_542 * fn004013F6(ui32 dwArg04, word32 dwArg08)
{
	struct Eq_542 * eax_26;
	struct Eq_546 * ecx_13 = dwArg04 + dwArg04->dw003C / 0x0040;
	struct Eq_542 * edx_16 = ecx_13->w0014 + 0x02 + (word32) ecx_13->w0014 / 22;
	struct Eq_542 * esi_19 = edx_16 + (word32) ecx_13->w0006;
	if (edx_16 != esi_19)
	{
		do
		{
			if (dwArg08 >= edx_16->dw000C && dwArg08 < edx_16->dw0008 + edx_16->dw000C)
			{
				eax_26 = edx_16;
				return eax_26;
			}
			edx_16 = edx_16 + 0x01;
		} while (edx_16 != esi_19);
	}
	eax_26 = null;
	return eax_26;
}

// 0040143A: Register byte fn0040143A(Stack word32 dwArg00)
byte fn0040143A(word32 dwArg00)
{
	if (fn00401B98() != 0x00)
	{
		word32 edx_32 = fs->ptr0018->dw0004;
l0040145B:
		__lock();
		word32 eax_36;
		__cmpxchg(globals->dw403338, edx_32, 0x00, out eax_36);
		__lock();
		word32 eax_43;
		__cmpxchg(globals->dw403338, edx_32, 0x00, out eax_43);
		if (eax_43 != 0x00)
		{
			if (edx_32 != eax_43)
				goto l0040145B;
			return 0x01;
		}
		else
		{
			return 0x00;
			return 0x00;
		}
	}
	else
		return 0x00;
}

// 0040146F: Register byte fn0040146F(Register word32 edx, Stack ui32 dwArg04)
byte fn0040146F(word32 edx, ui32 dwArg04)
{
	if (dwArg04 == 0x00)
		globals->b403354 = 0x01;
	fn004019FE(edx);
	fn00401C46();
	if (true)
	{
		fn00401C46();
		if (true)
			return 0x01;
		fn00401C46();
	}
	return 0x00;
}

// 004014A8: void fn004014A8(Register ptr32 ecx, Register ptr32 edi, Stack word32 dwArg04)
void fn004014A8(ptr32 ecx, ptr32 edi, word32 dwArg04)
{
	if (dwArg04 == 0x00 || dwArg04 == 0x01)
	{
		word32 eax_35 = fn00401B98();
		if (eax_35 != 0x00 && dwArg04 == 0x00)
		{
			word32 esp_125;
			word32 ebp_126;
			byte SCZO_127;
			word32 esi_128;
			byte SZO_129;
			byte C_130;
			byte Z_131;
			word32 eax_132;
			word32 ecx_133;
			byte al_134;
			word32 edi_135;
			byte cl_136;
			word32 ebx_137;
			initialize_onexit_table();
			if (eax_132 == 0x00)
			{
				fp->dwFFFFFFE4 = 0x00403348;
				word32 esp_149;
				word32 ebp_150;
				byte SCZO_151;
				word32 esi_152;
				byte SZO_153;
				byte C_154;
				byte Z_155;
				word32 eax_156;
				word32 ecx_157;
				byte al_158;
				word32 edi_159;
				byte cl_160;
				word32 ebx_161;
				initialize_onexit_table();
			}
		}
		else
		{
			ui32 eax_62 = globals->dw403004;
			ui32 eax_78 = __ror(eax_62 & 0x1F | ~0x00, (byte) (0x20 - (eax_62 & 0x1F))) ^ globals->dw403004;
			globals->dw40333C = eax_78;
			globals->dw403340 = eax_78;
			globals->dw403344 = eax_78;
			globals->dw403348 = eax_78;
			globals->dw40334C = eax_78;
			globals->dw403350 = eax_78;
		}
		return;
	}
	else
	{
		fn0040176D(dwLoc1C, 0x05);
		int3();
		word32 ebx_175;
		word32 esi_176;
		word32 edi_177;
		fn0040153F(ecx, ebx, dwArg04, edi, out ebx_175, out esi_176, out edi_177);
		return;
	}
}

// 0040153F: Register (ptr Eq_745) fn0040153F(Register ptr32 ecx, Register word32 ebx, Register ptr32 esi, Register ptr32 edi, Register out ptr32 ebxOut, Register out ptr32 esiOut, Register out ptr32 ediOut)
Eq_745 * fn0040153F(ptr32 ecx, word32 ebx, ptr32 esi, ptr32 edi, ptr32 & ebxOut, ptr32 & esiOut, ptr32 & ediOut)
{
	struct Eq_745 * eax_31;
	struct Eq_67 * ebp_11 = fn00401980(ecx, ebx, esi, edi, dwLoc0C, 0x00402528, 0x08);
	*(ebp_11 - 0x04) = *(ebp_11 - 0x04) & 0x00;
	struct Eq_765 * eax_16 = (struct Eq_765 *) 23117;
	if ((word16 *) 0x00400000 == 23117)
	{
		eax_16 = (struct Eq_765 *) (struct Eq_765 **) 0x0040003C;
		if (eax_16->dw400000 == 0x4550 && eax_16->w400018 == 0x010B)
		{
			*(fp - 0x0C) = ebp_11->dw0008 - 0x00400000;
			*(fp - 0x10) = 0x00400000;
			eax_16 = fn004013F6(dwArg00, dwArg04);
			if (eax_16 != null && eax_16->dw0024 >= 0x00)
			{
				*(ebp_11 - 0x04) = ~0x01;
				eax_31 = (struct Eq_745 *) DPB(eax_16, 0x01, 0);
l004015C3:
				word32 ebp_37;
				word32 esi_38;
				word32 edi_39;
				word32 ebx_40;
				*ebxOut = fn004019C6(ebp_11, 0x08, dwArg00, dwArg04, dwArg08, dwArg0C, out ebp_37, out esi_38, out edi_39);
				return eax_31;
			}
		}
	}
	*(ebp_11 - 0x04) = ~0x01;
	eax_31 = (struct Eq_745 *) DPB(eax_16, 0x00, 0);
	goto l004015C3;
}

// 004015C9: void fn004015C9(Stack byte bArg04)
void fn004015C9(byte bArg04)
{
	if (fn00401B98() != 0x00 && bArg04 == 0x00)
		globals->dw403338 = 0x00;
	return;
}

// 004015E6: void fn004015E6(Stack ui32 dwArg04, Stack byte bArg08)
void fn004015E6(ui32 dwArg04, byte bArg08)
{
	if (globals->b403354 == 0x00 || bArg08 == 0x00)
	{
		fn00401C46();
		fn00401C46();
	}
	return;
}

// 0040160E: Register word32 fn0040160E(Stack word32 dwArg04)
word32 fn0040160E(word32 dwArg04)
{
	word32 eax_24;
	ui32 eax_7 = globals->dw403004;
	byte cl_11 = (byte) (eax_7 & 0x1F);
	word32 eax_17 = __ror(eax_7 ^ globals->dw40333C, cl_11);
	if (eax_17 == ~0x00)
	{
		word32 esp_40;
		word32 ebp_41;
		word32 ecx_43;
		byte SZO_44;
		byte C_45;
		byte cl_46;
		byte SCZO_47;
		byte Z_48;
		crt_atexit();
	}
	else
	{
		word32 esp_52;
		word32 ebp_53;
		word32 ecx_55;
		byte SZO_56;
		byte C_57;
		byte cl_58;
		byte SCZO_59;
		byte Z_60;
		register_onexit_function();
	}
	return ~(0x00 - (-eax_24 == 0x00)) & dwArg04;
}

// 00401649: void fn00401649(Stack word32 dwArg04)
void fn00401649(word32 dwArg04)
{
	fn0040160E(dwArg04);
	return;
}

// 0040165E: Register ui32 fn0040165E()
ui32 fn0040165E()
{
	ui32 eax_16 = globals->dw403004;
	if (eax_16 != 0xBB40E64E && (eax_16 & 0xFFFF0000) != 0x00)
		globals->dw403000 = ~eax_16;
	else
	{
		GetSystemTimeAsFileTime(fp - 0x10);
		ui32 v14_55 = dwLoc0C & 0x00 ^ dwLoc10 & 0x00 ^ GetCurrentThreadId() ^ GetCurrentProcessId();
		QueryPerformanceCounter(fp - 0x18);
		ui32 ecx_69 = dwLoc14 ^ dwLoc18 ^ v14_55 ^ fp - 0x08;
		if (ecx_69 == 0xBB40E64E)
			ecx_69 = ~0x44BF19B0;
		else if ((ecx_69 & 0xFFFF0000) == 0x00)
			ecx_69 = ecx_69 | (ecx_69 | 0x4711) << 0x10;
		globals->dw403004 = ecx_69;
		ecx = ~ecx_69;
		globals->dw403000 = ecx;
	}
	return ecx;
}

// 004016FA: void fn004016FA()
void fn004016FA()
{
	return;
}

// 004016FE: void fn004016FE()
void fn004016FE()
{
	return;
}

// 00401704: void fn00401704()
void fn00401704()
{
	InitializeSListHead(&globals->u403358);
	return;
}

// 00401710: void fn00401710(Register word32 eax)
void fn00401710(word32 eax)
{
	word32 esp_8;
	byte SCZO_9;
	word32 eax_10;
	byte SZO_11;
	byte C_12;
	byte Z_13;
	controlfp_s();
	if (eax_10 != 0x00)
	{
		fp->dwFFFFFFF8 = 0x07;
		fn0040176D(0x00030000, dwArg00);
		int3();
		fn00401731();
		return;
	}
	else
		return;
}

// 00401731: void fn00401731()
void fn00401731()
{
	return;
}

// 00401732: void fn00401732()
void fn00401732()
{
	return;
}

// 00401738: void fn00401738()
void fn00401738()
{
	fn00401000();
	word32 ecx_3 = globals->dw40337C;
	globals->dw403378 = globals->dw403378 | 0x04;
	globals->dw40337C = ecx_3;
	fn00401732();
	word32 ecx_9 = globals->dw403364;
	globals->dw403360 = globals->dw403360 | 0x02;
	globals->dw403364 = ecx_9;
	return;
}

// 00401755: void fn00401755()
void fn00401755()
{
	return;
}

// 00401761: void fn00401761()
void fn00401761()
{
	return;
}

// 00401767: Register ptr32 fn00401767()
ptr32 fn00401767()
{
	return 0x00403380;
}

// 0040176D: void fn0040176D(Stack word32 dwArg00, Stack ui32 dwArg04)
void fn0040176D(word32 dwArg00, ui32 dwArg04)
{
	if (IsProcessorFeaturePresent(0x17) == 0x00)
	{
		globals->dw403368 = 0x00;
		memset(fp + ~0x0327, 0x00, 0x02CC);
		memset(fp - 0x5C, 0x00, 0x50);
		byte bl_90 = 0x00 - (0x01 - IsDebuggerPresent() == 0x00);
		SetUnhandledExceptionFilter(null);
		if (UnhandledExceptionFilter(fp - 0x0C) == 0x00)
			globals->dw403368 = globals->dw403368 & 0x00 - (-((word32) (bl_90 + 0x01)) == 0x00);
		return;
	}
	else
		__fastfail(dwArg04);
}

// 00401888: void fn00401888()
void fn00401888()
{
	return;
}

// 0040188B: Register byte fn0040188B()
byte fn0040188B()
{
	Eq_1103 eax_4 = GetModuleHandleW(null);
	if (eax_4 != null && eax_4->unused == 23117)
	{
		struct Eq_1117 * eax_42 = eax_4 + eax_4->dw003C / 0x0040;
		if (eax_42->dw0000 == 0x4550 && (eax_42->w0018 == 0x010B && eax_42->dw0074 > 0x0E))
			return fn004018CB();
	}
	return 0x00;
}

// 004018CB: Register byte fn004018CB()
byte fn004018CB()
{
	return Z;
}

// 004018CF: void fn004018CF()
void fn004018CF()
{
	SetUnhandledExceptionFilter(&globals->t4018DB);
	return;
}

// 0040191C: void fn0040191C()
void fn0040191C()
{
	word32 * esi_34 = &globals->dw4024FC;
	if (false)
	{
		do
		{
			<anonymous> * edi_37 = *esi_34;
			if (edi_37 != null)
			{
				fn00401972();
				word32 esp_54;
				word32 ebx_55;
				byte SCZO_57;
				byte C_58;
				word32 edi_59;
				byte SZO_60;
				byte Z_61;
				word32 ecx_62;
				edi_37();
			}
			esi_34 = esi_34 + 0x01;
		} while (esi_34 < &globals->dw4024FC);
	}
	return;
}

// 00401972: void fn00401972()
void fn00401972()
{
	word32 esp_3;
	globals->ptr4020D4();
	return;
}

// 00401980: Register ptr32 fn00401980(Register ptr32 ecx, Register word32 ebx, Register ptr32 esi, Register ptr32 edi, Stack word32 dwArg00, Stack word32 dwArg04, Stack word32 dwArg08)
ptr32 fn00401980(ptr32 ecx, word32 ebx, ptr32 esi, ptr32 edi, word32 dwArg00, word32 dwArg04, word32 dwArg08)
{
	ptr32 esp_14 = fp - 0x08 - dwArg08;
	*(esp_14 - 0x04) = ebx;
	*(esp_14 - 0x08) = esi;
	*(esp_14 - 0x0C) = edi;
	ui32 eax_24 = globals->dw403004;
	*(esp_14 - 0x10) = eax_24 ^ fp + 0x08;
	*(esp_14 - 0x14) = dwArg00;
	fs->ptr0000 = fp - 0x08;
	ui32 v11_26 = dwArg04 ^ eax_24;
	if (ecx == 0x00)
	{
		ptr32 ebp_50;
		word32 esi_51;
		word32 edi_52;
		fn004019C6(fp + 0x08, 4200923, v11_26, ~0x01, ebp, dwArg0C, out ebp_50, out esi_51, out edi_52);
		return ebp_50;
	}
	else
		return fp + 0x08;
}

// 004019C6: Register word32 fn004019C6(Register (ptr Eq_67) ebp, Stack word32 dwArg00, Stack ui32 dwArg04, Stack word32 dwArg08, Stack ptr32 dwArg0C, Stack word32 dwArg10, Register out ptr32 ebpOut, Register out ptr32 esiOut, Register out ptr32 ediOut)
word32 fn004019C6(Eq_67 * ebp, word32 dwArg00, ui32 dwArg04, word32 dwArg08, ptr32 dwArg0C, word32 dwArg10, ptr32 & ebpOut, ptr32 & esiOut, ptr32 & ediOut)
{
	fs->dw0000 = *(ebp - 0x10);
	word32 ebp_22 = ebp->dw0000;
	*ebpOut = ebp_22;
	ebp->dw0000 = dwArg00;
	word32 edi_14 = dwArg08;
	*ediOut = edi_14;
	word32 esi_17 = dwArg0C;
	*esiOut = esi_17;
	if (dwArg00 == 0x00)
	{
		*(ebp - 0x04) = ebp_22;
		*(ebp - 0x08) = ebp->dw0010;
		*(ebp - 0x0C) = ebp->dw000C;
		*(ebp - 0x10) = ebp->dw0008;
		*(ebp - 0x14) = ebp->dw0004;
		*(ebp - 0x18) = 0x0040108B;
		*(ebp - 0x1C) = 0x00403004;
		struct Eq_1308 * esp_57;
		word32 ecx_58;
		word32 ebp_59;
		struct Eq_1311 * fs_60;
		word32 edi_61;
		word32 esi_62;
		word32 ebx_63;
		byte SCZO_64;
		except_handler4_common();
		word32 ebp_68;
		*ebpOut = esp_57->dw0014;
		return ebx_63;
	}
	else
		return dwArg10;
}

// 004019FE: void fn004019FE(Register word32 edx)
void fn004019FE(word32 edx)
{
	globals->dw40336C = globals->dw40336C & 0x00;
	globals->dw403010 = globals->dw403010 | 0x01;
	if (IsProcessorFeaturePresent(0x0A) != 0x00)
	{
		ui32 edi_135;
		globals->dw403010 = globals->dw403010 | 0x02;
		globals->dw40336C = 0x01;
		int32 eax_89;
		word32 ebx_90;
		word32 ecx_91;
		word32 edx_92;
		__cpuid(0x00, 0x00, &eax_89, &ebx_90, &ecx_91, &edx_92);
		ui32 eax_124;
		word32 ebx_125;
		ui32 ecx_126;
		word32 edx_127;
		__cpuid(0x01, 0x00, &eax_124, &ebx_125, &ecx_126, &edx_127);
		if ((edx_92 ^ 0x49656E69 | ecx_91 ^ 1818588270 | ebx_90 ^ 1970169159) == 0x00 && ((eax_124 & 0x0FFF3FF0) == 0x000106C0 || ((eax_124 & 0x0FFF3FF0) == 0x00020660 || ((eax_124 & 0x0FFF3FF0) == 0x00020670 || ((eax_124 & 0x0FFF3FF0) == 0x00030650 || ((eax_124 & 0x0FFF3FF0) == 0x00030660 || (eax_124 & 0x0FFF3FF0) == 0x00030670))))))
		{
			ui32 edi_245 = globals->dw403370;
			globals->dw403370 = edi_245 | 0x01;
			edi_135 = edi_245 | 0x01;
		}
		else
			edi_135 = globals->dw403370;
		if (eax_89 >= 0x07)
		{
			word32 eax_210;
			ui32 ebx_211;
			word32 ecx_212;
			word32 edx_213;
			__cpuid(0x07, 0x00, &eax_210, &ebx_211, &ecx_212, &edx_213);
			if ((ebx_211 & 0x0200) != 0x00)
				globals->dw403370 = edi_135 | 0x02;
		}
		if ((ecx_126 & 0x00100000) != 0x00)
		{
			globals->dw403010 = globals->dw403010 | 0x04;
			globals->dw40336C = 0x02;
			if ((ecx_126 & 0x08000000) != 0x00 && ((ecx_126 & 0x10000000) != 0x00 && (((word32) __xgetbv(0x00) & 0x06) == 0x06 && true)))
			{
				ui32 eax_189 = globals->dw403010;
				globals->dw40336C = 0x03;
				globals->dw403010 = eax_189 | 0x08;
				if ((bLoc14 & 0x20) != 0x00)
				{
					globals->dw40336C = 0x05;
					globals->dw403010 = eax_189 | 0x08 | 0x20;
				}
			}
		}
	}
	return;
}

// 00401B98: Register word32 fn00401B98()
word32 fn00401B98()
{
	return (word32) (globals->dw403014 != 0x00);
}

// 00401C46: void fn00401C46()
void fn00401C46()
{
	return;
}

