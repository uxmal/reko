// test.c
// Generated by decompiling test.exe
// using Reko decompiler version 0.8.1.0.

#include "test.h"

// 00011000: Register word32 fn00011000(Register out ptr32 raOut)
word32 fn00011000(ptr32 & raOut)
{
	memset(fp + -0x0014, 0x00, 0x05);
	memcpy(calloc(0x01, 0x05), fp + ~0x1B, 0x05);
	dwLoc24->b0000 = 0x0C;
	dwLoc24->dw0001 = 0x00;
	raOut = dwLoc14;
	return 0x00;
}

// 000110A0: Register Eq_n Win32CrtStartup()
Eq_n Win32CrtStartup()
{
	fn000111C4(ra);
	return fn000110E8(ra);
}

// 000110E8: Register Eq_n fn000110E8(Register word32 ra)
Eq_n fn000110E8(word32 ra)
{
	fn000114E4(ra);
	word32 ra_n;
	Eq_n r2_n = fn00011000(out ra_n);
	fn00011278(r2_n, ra_n);
	return r2_n;
}

// 0001116C: void fn0001116C(Register (ptr32 Eq_n) r2, Register (ptr32 (ptr32 word32)) r4, Register word32 r30, Register word32 ra)
void fn0001116C(struct Eq_n * r2, word32 ** r4, word32 r30, word32 ra)
{
	r2->ptrFFFFFFEC = r4;
	r2->ptrFFFFFFF0 = r2->ptrFFFFFFEC;
	r2->dwFFFFFFF4 = **r2->ptrFFFFFFEC;
	word32 r8_n = r2->dwFFFFFFF4;
	r2->dwFFFFFFE8 = r8_n;
	XcptFilter();
}

// 000111C4: void fn000111C4(Register word32 ra)
void fn000111C4(word32 ra)
{
	ui32 r8_n = globals->dw13030;
	if (r8_n != 0x00 && r8_n != 0xB064)
		globals->dw13034 = ~r8_n;
	else
	{
		uint32 r2_n;
		COREDLL.dll!Ordinal_n();
		ui32 r8_n = r2_n >> 0x10 ^ r2_n + 0xFFFF;
		ui32 dwLoc0C_n = r8_n;
		if (r8_n == 0x00)
			dwLoc0C_n = 0xB064;
		globals->dw13030 = dwLoc0C_n;
		globals->dw13034 = ~dwLoc0C_n;
	}
}

// 00011278: void fn00011278(Register Eq_n r4, Register word32 ra)
void fn00011278(Eq_n r4, word32 ra)
{
	fn000112A8(r4, 0x00, 0x00, ra);
}

// 000112A8: void fn000112A8(Register Eq_n r4, Register int32 r5, Register int32 r6, Register word32 ra)
void fn000112A8(Eq_n r4, int32 r5, int32 r6, word32 ra)
{
	globals->b13038 = (byte) (r6 << 0x18 >> 0x18);
	if (r5 != 0x00)
	{
		fn0001147C(&globals->dw12018, &globals->dw1201C, ra);
		if (r6 != 0x00)
			return;
		TerminateProcess(fn00011460(), r4);
	}
	else if (globals->t13040 == 0x00)
	{
l00011374:
		fn0001147C(&globals->dw12010, &globals->dw12014, ra);
	}
	else
	{
		do
		{
			globals->t1303C = (word32) globals->t1303C - 0x04;
			if ((word32) (globals->t1303C < globals->t13040) != 0x00)
			{
				free(globals->t13040);
				globals->t1303C.u0 = 0x00;
				globals->t13040 = globals->t1303C;
				goto l00011374;
			}
		} while (*globals->t1303C == null);
		<anonymous> * r8_n = *globals->t1303C;
		r8_n();
	}
}

// 000113D8: void fn000113D8(Register Eq_n r4, Register word32 ra)
void fn000113D8(Eq_n r4, word32 ra)
{
	fn000112A8(r4, 0x01, 0x00, ra);
}

// 00011408: void fn00011408(Register word32 ra)
void fn00011408(word32 ra)
{
	fn000112A8(0x00, 0x00, 0x01, ra);
}

// 00011434: void fn00011434(Register word32 ra)
void fn00011434(word32 ra)
{
	fn000112A8(0x00, 0x01, 0x01, ra);
}

// 00011460: Register int32 fn00011460()
int32 fn00011460()
{
	return 66;
}

// 0001147C: void fn0001147C(Register (ptr32 word32) r4, Register (ptr32 word32) r5, Register word32 ra)
void fn0001147C(word32 * r4, word32 * r5, word32 ra)
{
	word32 * dwArg00_n = r4;
	while ((word32) (dwArg00_n < r5) != 0x00)
	{
		if (*dwArg00_n != 0x00)
			fn00000000();
		++dwArg00_n;
	}
}

// 000114E4: void fn000114E4(Register word32 ra)
void fn000114E4(word32 ra)
{
	fn0001147C(&globals->dw12008, &globals->dw1200C, ra);
	fn0001147C(&globals->dw12000, &globals->dw12004, ra);
}

// 0001152C: Register word32 fn0001152C(Register word32 ra)
word32 fn0001152C(word32 ra)
{
	Eq_n r8_n = globals->t13040;
	struct Eq_n * sp_n = fp + -0x0038;
	Eq_n dwLoc20_n = 0x00;
	Eq_n r8_n = globals->t1303C - r8_n;
	if (r8_n >= 0x00)
	{
		if (r8_n != 0x00)
		{
			Eq_n r2_n;
			word32 r5_n;
			msize();
			dwLoc20_n = r2_n;
		}
		if ((word32) (dwLoc20_n < (word32) r8_n + 0x04) != 0x00)
		{
			if (r8_n == 0x00)
			{
				malloc(0x0010);
				sp_n = fp + ~0x3B;
			}
			else
			{
				Eq_n dwLoc18_n = dwLoc20_n << 0x01;
				if ((word32) (dwLoc20_n < 0x0201) == 0x00)
					dwLoc18_n = (word32) dwLoc20_n + 0x0200;
				if ((word32) (dwLoc20_n < dwLoc18_n) != 0x00)
				{
					realloc(r8_n, dwLoc18_n);
					sp_n = fp + ~0x3B;
				}
				if (sp_n->t0014 == 0x00 && (word32) (sp_n->t0018 < sp_n->t0024) != 0x00)
				{
					sp_n = (struct Eq_n *) ((char *) sp_n - 0x04);
					sp_n->t0014 = realloc(sp_n->t001C, sp_n->t0024);
				}
			}
			if (sp_n->t0014 == 0x00)
			{
				sp_n->ptr002C = null;
				return dwLoc0C;
			}
			sp_n->t0028 = (word32) sp_n->t0014 + ((sp_n->t0028 - sp_n->t001C >> 0x02) << 0x02);
			sp_n->t001C = sp_n->t0014;
		}
		*sp_n->t0028 = sp_n->ptr0038;
		sp_n->t0028 = (word32) sp_n->t0028 + 0x04;
		globals->t1303C = sp_n->t0028;
		globals->t13040 = sp_n->t001C;
		sp_n->ptr002C = sp_n->ptr0038;
		return dwLoc0C;
	}
	else
	{
		dwLoc0C = 0x00;
		return dwLoc0C;
	}
}

// 000116FC: void fn000116FC(Register word32 ra)
void fn000116FC(word32 ra)
{
	fn0001152C(ra) == 0x00;
}

