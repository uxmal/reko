...
{
	word56 rsi_56_n_n = SLICE(rsi, word56, 8);
	byte al = (byte) rax;
	...
	if (*rax_n != null)
	{
		...
		word56 rax_56_n_n = SLICE(rax_n, word56, 8);
		if (al_n != 0x00)
		{
	...
	if (*rax_n != null)
	{
		...
		al_n = (byte) rax_n;
		rax_56_n_n = SLICE(rax_n, word56, 8);
		if (al_n != 0x00)
		{
			word64 rax_n;
			word64 r8_n;
...
{
	word32 rax_32_n_n = SLICE(SetUnhandledExceptionFilter(null), word32, 32);
	UnhandledExceptionFilter(rcx);
	TerminateProcess(SEQ(rax_32_n_n, GetCurrentProcess()), 0xC0000409);
}
	...
	if (globals->w40000000 == 23117)
	{
		...
		if (rcx_n->dw0000 == 0x4550)
		{
			...
			if (rcx_n->w0018 == 0x020B)
			{
				...
				uint56 rax_56_n_n = (uint56) (uint24) SLICE(eax_n, word24, 8);
...
uint64 fn0000000140001974(word32 ecx, word64 qwArg00, word64 qwArg10, ptr64 & rcxOut)
{
	...
	if (IsProcessorFeaturePresent(0x17) == 0x00)
	{
		...
		uint64 rax_n = SEQ(rax_32_n_n, eax_n);
		if (eax_n == 0x00)
		{
...
void fn0000000140001BFC(word32 edx, Eq_n ebx)
{
	...
	ui32 rdx_32_n_n = SLICE(rdx_n, word32, 32);
...
l0000000140001CE9:
	...
	if (!__bt(0x00, 0x14))
	{
		...
		if (!__bt(0x00, 0x1B) && !__bt(0x00, 0x1C))
		{
			...
			if (((byte) (SEQ(rdx_32_n_n, SLICE(edx_eax_n, word32, 32)) << 0x20 | (uint64) ((uint32) ((word32) edx_eax_n))) & 0x06) == 0x06)
			{
