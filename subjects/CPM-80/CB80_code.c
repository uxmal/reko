	...
	*g_t1678 = 0x00;
	...
	while (g_t167A <= 0x03)
	{
		...
		while (true)
		{
			word16 de_n;
			word16 hl_n;
			Eq_n C_n = fn1348(&g_t1676, g_t167A * 0x02 + 0x0227, out de_n, out hl_n);
			if (C_n)
				break;
			struct Eq_n * de_n;
			word16 hl_n;
			fn1348(&g_t1676, g_t167A * 0x02 + 0x0227, out de_n, out hl_n);
		...
		if (__rcr(g_b167B, 0x01, C_n) < 0x00)
		{
			...
			((word16) globals->t1678 + (uint16) ((uint8) ((byte) a_n.t0000 + 1)))->u0 = g_b167C;
		}
		g_t167A = (word16) g_t167A + 1;
		if (g_t167A == 0x00)
			break;
	}
	Eq_n hl_n = g_t1678;
...
void fn07B3(Eq_n b)
{
	...
	if (__rcr(g_b14F9, 0x01, C) < 0x00)
	{
		...
		fn05CE(0x02FC);
		fn06CE(0x00, g_t14FE);
		globals->t14FE.u1 = (word16) g_t14FE + 1;
		byte b_n;
		fn05EF(898, out b_n);
	}
	g_b14FD = 0x03;
...
uint8 fn0920()
{
	...
	if (g_b168D == 0x00)
	{
		...
		if ((~(0x00 - (a_n < 0x30)) & g_b168E) >> 0x01 < 0x00)
			((word16) globals->t168B + 7)->u0 = *((word16) g_t168B + 7) | 0x80;
		a_n = F_OPEN(g_t168B);
		if (a_n == ~0x00)
		{
			Eq_n hl_n = g_t1687;
...
Eq_n fn0990(byte f, byte b, Eq_n c, byte d, Eq_n e, union Eq_n & afOut)
{
	...
	while (true)
	{
		*((word16) sp_n - 2) = SEQ(~(0x00 - (g_t1697 > 0x07)), f);
		word16 af_n = fn0B74();
		f = (byte) af_n;
	...
	byte b_n;
	cu8 * hl_n = (word16) g_t1692 + (uint16) ((uint8) g_t1697);
	...
	*((word16) sp_n - 2) = SEQ(b_n, 0x03);
	...
	while (g_t1697 <= 0x07)
	{
		((word16) globals->t168B + ((word16) globals->t1697 + 1))->u0 = fn03E6(*((word16) g_t168B + ((word16) g_t1697 + 1)));
		Eq_n a_n = g_t1697;
		g_t1697 = (word16) a_n + 1;
		if (a_n == 0x01)
			break;
	}
	globals->t1697.u0 = 0x00;
	do
	{
		Eq_n C_n = (bool) cond(0x02 - g_t1697);
		if (g_t1697 > 0x02)
			break;
		byte a_n = fn03E6(*((word16) g_t168B + ((word16) g_t1697 + 9)));
		byte * hl_n = (word16) g_t168B + ((word16) g_t1697 + 9);
		*hl_n = a_n;
		Eq_n a_n = g_t1697;
		g_t1697 = (word16) a_n + 1;
		C_n = (bool) cond(hl_n);
...
void fn0E63(byte c)
{
	...
	while (true)
	{
		...
		if ((fn1353(0x00, &g_ptr1521, out l_n) | (byte) hl_n) == 0x00)
			break;
		if (g_b16A6 == (globals->ptr1521)->b0002)
		{
			cu8 a_n;
...
bool fn0EAB(union Eq_n & aOut)
{
	...
	{
		Eq_n a_n = g_t138C;
		bool C_n = (bool) cond(a_n - g_t16A7);
		if (a_n < g_t16A7)
			break;
		Eq_n hl_n = g_t16A7;
		byte a_n = g_ptr1521[((word16) g_t16A7 + 0x0A) / 0x0A];
...
void fn100A(word16 af)
{
	...
	{
		byte f_n = (byte) af;
		if ((0x00 - (g_b14FF < 0x0E) & SLICE(af, byte, 8)) >> 0x01 >= 0x00)
			break;
		cu8 a_n = g_b14FF;
		g_b14FF = a_n + 0x01;
