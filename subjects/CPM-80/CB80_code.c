...
{
	g_t1657 = c;
	byte a_n = 0x00 - (g_t1657 > 0x7B || g_t1657 < 0x61);
	aOut = ~a_n;
...
bool fn05EF(Eq_n bc, union Eq_n & bOut)
{
	...
	do
	{
		Eq_n a_n = *g_t166F;
		byte b_n = SLICE(bc_n, byte, 8);
		bool C_n = (bool) cond(a_n - g_t1671);
		if (a_n < g_t1671)
			break;
		C_n = fn056B(SEQ(b_n, *((word16) g_t166F + (uint16) ((byte) g_t1671))), out bc_n);
		g_t1671.u1 = (word16) g_t1671 + 1;
	} while (g_t1671 != 0x00);
	bOut.u1 = <invalid>;
	return C_n;
}
...
void fn075C(byte b, Eq_n c, byte d, Eq_n e)
{
	...
	Eq_n hl_n = g_t1683;
	word16 bc_n;
	fn056B(SEQ(SLICE(SEQ(0x00, fn06CE(SLICE(hl_n, byte, 8), (byte) hl_n)), byte, 8), 0x0D), out bc_n);
	word16 bc_n;
	fn056B(SEQ(SLICE(bc_n, byte, 8), 0x0A), out bc_n);
	byte l_n;
...
Eq_n fn0990(byte f, byte b, Eq_n c, byte d, Eq_n e, union Eq_n & afOut)
{
	...
	if (*((word16) hl_n + 2) == 0x3A)
	{
		...
		sp_n->tFFFFFFFE = af_n;
		f = (byte) af_n;
		sp_n->tFFFFFFFE = SEQ(0x00 - (*hl_n < 0x41) | sp_n->b0003, f);
		bcu8 a_n = 0x00 - (g_t1696 > 0x02) & sp_n->b0003;
	...
	while (true)
	{
		sp_n->tFFFFFFFE = SEQ(~(0x00 - (g_b1697 > 0x07)), f);
		word16 af_n = fn0B74();
		f = (byte) af_n;
	...
	sp_n->tFFFFFFFE = SEQ(0x00 - (g_t1696 > 0x01), f);
...
void fn100A(word16 af)
{
	...
	{
		byte f_n = (byte) af;
		if ((0x00 - (g_b14FF < 0x0E) & (0x00 - (g_t16A8 < ~0x00) & (0x00 - (g_t16A8 < 0x03) & SLICE(af, byte, 8)))) >> 0x01 >= 0x00)
			break;
		cu8 a_n = g_b14FF;
		g_b14FF = a_n + 0x01;
		(&g_b14FF)[(uint16) (a_n + 0x01)] = (cu8) g_t16A8;
		g_t16A8 = fn045B();
		af = SEQ(0x00 - (g_t16A8 < 33), f_n);
	}
	if (g_b14FF == 0x00)
	{
		fn05CE(0x0369);
	...
	while ((0x00 - (g_t16A8 < ~0x00) & 0x00 - (g_t16A8 < 0x03)) >> 0x01 < 0x00)
		g_t16A8 = fn045B();
	g_t16A8 = fn045B();
	while (true)
	{
		...
		if (g_t16A8 == 66)
			g_b14F3 = 0x00;
		else if (g_t16A8 == 0x43)
		{
			word16 af_n = fn12D8();
			g_t151D.t0000 = SLICE(af_n, byte, 8);
			f_n = (byte) af_n;
		}
		else if (g_t16A8 == 0x44)
		{
			word16 af_n = fn1279();
			g_b138B = SLICE(af_n, byte, 8);
			f_n = (byte) af_n;
		}
		else if (g_t16A8 == 0x46)
			g_b14F3 = 0x04;
		else if (g_t16A8 == 0x49)
			g_b14FA = 0x01;
		else if (g_t16A8 == 0x4C)
		{
			word16 af_n = fn1279();
			g_b14F7 = SLICE(af_n, byte, 8);
			f_n = (byte) af_n;
		}
		else if (g_t16A8 == 0x4E)
			g_b14FB = 0x01;
		else if (g_t16A8 == 0x4F)
			g_b14EF = 0x00;
		else if (g_t16A8 == 0x50)
			g_b14F3 = 0x01;
		else if (g_t16A8 == 0x52)
		{
			word16 af_n = fn12D8();
			g_t1520.t0000 = SLICE(af_n, byte, 8);
			f_n = (byte) af_n;
		}
		else if (g_t16A8 == 0x53)
			g_b14F0 = 0x01;
		else if (g_t16A8 == 0x54)
			g_b14F5 = 0x01;
		else if (g_t16A8 == 0x55)
			g_b1529 = 0x01;
		else if (g_t16A8 == 0x56)
			g_b14FC = 0x01;
		else if (g_t16A8 == 0x57)
		{
			word16 af_n = fn1279();
...
void fn1229()
{
	...
	if (g_t16A8 != 0x00)
	{
		...
		while (true)
		{
			...
			if (g_t16A8 == 0x00)
				break;
			fn056B(SEQ(b_n, (byte) g_t16A8), out bc_n);
			g_t16A8 = fn045B();
		}
	}
	fn0387();
...
word16 fn1279()
{
	...
	while (true)
	{
		...
		if (a_n > 0x39 || a_n < 0x30)
			break;
		g_b16AA = (word16) g_t16A9 + 0x0F + (g_b16AA * 0x02 + g_b16AA * 0x08);
	}
	if (g_t16A9 == 0x20)
		fn1262();
	if (g_t16A9 != 0x29)
