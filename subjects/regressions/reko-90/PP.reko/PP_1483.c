...
Eq_n fn1483-0C11(byte * ds_si, word16 * es_di, Eq_n al, word16 cx, Eq_n bx, Eq_n bp)
{
	...
	if (al - 0x8C - (al < ss->*si) != 0x00 && cx != 0x01)
	{
		...
		else
		{
			...
			word16 di_n;
			struct Eq_n * es_n;
			return (word16) (uint32) (uint16) fn1483-0C91(bx, bp, ax, cx_n - 0x01, (word32) si_n + 1, (word16) di_n + 1, stackArg0, out di_n, out es_n);
		}
	}
	else
		*es_di = 0x1955;
	...
	else
	{
		...
		if (v37_n < 0x00)
		{
			while (true)
			{
				...
				byte bl_n = (byte) bx_n;
				byte bh_n = SLICE(bx_n, byte, 8);
				sp_n = (union Eq_n Eq_n::*) ((char *) fp + 420);
				dl_n = (byte) dx_n;
				byte cl_n = (byte) cx_n;
				if (!CZ_n)
					break;
				...
				CZ_n.u0 = SLICE(cond((char *) fp + 0x00008FAA - ((char *) fp + 394)), bool, 2);
				--cx_n;
