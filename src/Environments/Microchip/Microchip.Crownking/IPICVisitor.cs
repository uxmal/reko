...
{
    #region Memory Traits visitors

    /// <summary>
    /// The <see cref="IMemTraitsSymbolVisitor"/> defines the interface for the implementation of the memory traits visitor methods.
    /// </summary>
    /// 
    ...
    /// <summary>
    /// The <see cref="IMemTraitsSymbolVisitor{T}"/> defines the interface for the implementation of the memory traits visitor functions.
    /// </summary>
    /// <typeparam name="T">Generic type parameter of the function's return value.</typeparam>
    public interface IMemTraitsSymbolVisitor<T>
    {
        T Visit(CalDataMemTraits mTraits);
        T Visit(CodeMemTraits mTraits);
        T Visit(ConfigFuseMemTraits mTraits);
        T Visit(ExtCodeMemTraits mTraits);
        T Visit(EEDataMemTraits mTraits);
        T Visit(BackgroundDebugMemTraits mTraits);
        T Visit(ConfigWORMMemTraits mTraits);
        T Visit(DataMemTraits mTraits);
        T Visit(DeviceIDMemTraits mTraits);
        T Visit(TestMemTraits mTraits);
        T Visit(UserIDMemTraits mTraits);
    }

    /// <summary>
    /// The <see cref="IMemTraitsSymbolVisitor{T, C}"/> defines the interface for the implementation of the memory traits visitor functions with context.
    /// </summary>
    /// <typeparam name="T">Generic type parameter of the function's return value.</typeparam>
    /// <typeparam name="C">Generic type parameter of the function's context parameter.</typeparam>
    public interface IMemTraitsSymbolVisitor<T, C>
    {
        T Visit(CalDataMemTraits mTraits, C context);
        T Visit(CodeMemTraits mTraits, C context);
        T Visit(ConfigFuseMemTraits mTraits, C context);
        T Visit(ExtCodeMemTraits mTraits, C context);
        T Visit(EEDataMemTraits mTraits, C context);
        T Visit(BackgroundDebugMemTraits mTraits, C context);
        T Visit(ConfigWORMMemTraits mTraits, C context);
        T Visit(DataMemTraits mTraits, C context);
        T Visit(DeviceIDMemTraits mTraits, C context);
        T Visit(TestMemTraits mTraits, C context);
        T Visit(UserIDMemTraits mTraits, C context);
    }

    /// <summary>
    /// The <see cref="IMemTraitsSymbolAcceptor"/> defines the interface for memory trait accepting a visitor method.
    /// </summary>
    ...
    /// <summary>
    /// The <see cref="IMemTraitsSymbolAcceptor{T}"/> defines the interface for memory trait accepting a visitor function.
    /// </summary>
    /// <typeparam name="T">Generic type parameter of the visitor function's return value.</typeparam>
    public interface IMemTraitsSymbolAcceptor<T>
    {
        /// <summary>
        /// The <see cref="Accept"/> function accepts a memory trait visitor and calls the appropriate
        /// "Visit()" function for this trait.
        /// </summary>
        /// <param name="v">An <see cref="IMemTraitsSymbolVisitor{T}"/> visitor to accept.</param>
        /// <returns>
        /// A value of type T.
        /// </returns>
        T Accept(IMemTraitsSymbolVisitor<T> v);

    }

    /// <summary>
    /// The <see cref="IMemTraitsSymbolAcceptor{T, C}"/> defines the interface for memory trait accepting a visitor function with a given context.
    /// </summary>
    /// <typeparam name="T">Generic type parameter of the visitor function's return value.</typeparam>
    /// <typeparam name="C">Generic type parameter of the function's context parameter.</typeparam>
    public interface IMemTraitsSymbolAcceptor<T, C>
    {
        /// <summary>
        /// The <see cref="Accept"/> function accepts a memory trait visitor and calls the appropriate
        /// "Visit()" function for this trait with the given context.
        /// </summary>
        /// <param name="v">An <see cref="IMemTraitsSymbolVisitor{T, C}"/> visitor to accept.</param>
        /// <param name="context">The context of generic type C.</param>
        /// <returns>
        /// A returned value of generic type T.
        /// </returns>
        T Accept(IMemTraitsSymbolVisitor<T, C> v, C context);
