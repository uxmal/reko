...
internal class GameboyInstructionComparer : InstructionComparer
{
    private const int MemCode = 0x10;
    private const int CondCode = 0x11;
    private const int StackCode = 0x11;

    public GameboyInstructionComparer(Normalize norm) : base(norm)
    {
    }
    ...
    public override bool DoCompareOperands(MachineOperand op1, MachineOperand op2)
    {
        return op1 switch
        {
            ...
            ConditionOperand<CCode> cc1 => cc1.Condition == ((ConditionOperand<CCode>) op2).Condition,
            StackOperand s1 => CompareStackOperands(s1, (StackOperand) op2),
            _ => throw new NotImplementedException(op1.GetType().Name)
        };
    }

    private bool CompareStackOperands(StackOperand s1, StackOperand s2)
    {
        return CompareConstants(s1.Offset, s2.Offset);
    }
    ...
    public override int GetOperandHash(MachineOperand op)
    {
        return op switch
        {
            ...
            StackOperand s => GetStackHash(s),
            _ => throw new NotImplementedException(op.GetType().Name)
        };
    }

    private int GetStackHash(StackOperand s)
    {
        int hash = StackCode;
        hash = hash * 17 ^ GetConstantHash(s.Offset);
        return hash;
    }
    ...
    private int GetMemoryHash(MemoryOperand m)
    {
        ...
        hash = hash * 3 ^ (m.PostIncrement ? 2 : 0);
