...
STDMETHODIMP Arm64Rewriter::Next()
{
	Dump("Next: %08x", this);
	if (available == 0)
...
HExpr Arm64Rewriter::FlagGroup(FlagM bits, const char * name, BaseType type)
{
	return host->EnsureFlagGroup(ARM64_REG_NZCV, (int) bits, name, type);
}
