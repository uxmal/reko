...
:00000001FF
";
            var hex = new HexLoader(sc, new RekoUri("file:foo.text"), Encoding.ASCII.GetBytes(data));
...
:00000001FF
";
            var hex = new HexLoader(sc, new RekoUri("foo.text"), Encoding.ASCII.GetBytes(data));
            var arch = new FakeArchitecture(sc);
            var program = hex.LoadProgram(Address.Ptr32(0x00), arch, new DefaultPlatform(sc, arch));
