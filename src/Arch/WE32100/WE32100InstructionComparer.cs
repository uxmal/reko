...
internal class WE32100InstructionComparer : InstructionComparer
{
    private const int MemCode = 0x10;

    public WE32100InstructionComparer(Normalize norm) : base(norm)
    {
    }
    ...
    private bool CompareMemoryOperands(MemoryOperand m1, MemoryOperand m2)
    {
        ...
        if (!CompareConstants(m1.Offset, m2.Offset))
            return false;
        return m1.Deferred == m2.Deferred;
    }
    ...
    private int GetMemoryHash(MemoryOperand m)
    {
        ...
        hash = hash * 5 ^ GetConstantHash(m.Offset);
        hash = hash * 3 ^ (m.Deferred ? 1 : 0);
        return hash;
    }
}