...
{
    private static readonly WideDecoder<SxAuroraDisassembler, Mnemonic, SxAuroraInstruction> rootDecoder;
    ...
    public SxAuroraDisassembler(SxAuroraArchitecture arch, EndianImageReader rdr)
    {
        this.arch = arch;
        this.rdr = rdr;
        this.ops = [];
    ...
    {
        return new SxAuroraInstruction
        {
    ...
    {
        this.address = rdr.Address;
        if (!rdr.TryReadLeUInt64(out ulong uInstr))
            return null;
        var instr = rootDecoder.Decode(uInstr, this);
        instr.Address = this.address;
        instr.Length = (int) (rdr.Address - this.address);
        return instr;
    }
    ...
    public override SxAuroraInstruction NotYetImplemented(string message)
    {
        ...
        testGenSvc?.ReportMissingDecoder("SxAuroraDis", this.address, this.rdr, message);
        return new SxAuroraInstruction
        {
            Mnemonic = Mnemonic.Invalid,
            InstructionClass = InstrClass.Invalid,
    ...
    private static bool CF(ulong uInstr, SxAuroraDisassembler dasm) => throw new NotImplementedException();
    private static bool RM(ulong uInstr, SxAuroraDisassembler dasm) => throw new NotImplementedException();
    private static bool RR(ulong uInstr, SxAuroraDisassembler dasm) => throw new NotImplementedException();
    private static bool RRM(ulong uInstr, SxAuroraDisassembler dasm) => throw new NotImplementedException();
    private static bool RV(ulong uInstr, SxAuroraDisassembler dasm) => throw new NotImplementedException();
    private static bool RVM(ulong uInstr, SxAuroraDisassembler dasm) => throw new NotImplementedException();
    private static bool RW(ulong uInstr, SxAuroraDisassembler dasm) => throw new NotImplementedException();

    private static WideDecoder<SxAuroraDisassembler, Mnemonic, SxAuroraInstruction> Instr(Mnemonic mnemonic, params WideMutator<SxAuroraDisassembler>[] mutators)
    {
        return new WideInstrDecoder<SxAuroraDisassembler, Mnemonic, SxAuroraInstruction>(InstrClass.Linear, mnemonic, mutators);
    }

    private static WideDecoder<SxAuroraDisassembler, Mnemonic, SxAuroraInstruction> Instr(Mnemonic mnemonic, InstrClass iclass,  params WideMutator<SxAuroraDisassembler>[] mutators)
    {
        return new WideInstrDecoder<SxAuroraDisassembler, Mnemonic, SxAuroraInstruction>(iclass, mnemonic, mutators);
    }
    ...
    static SxAuroraDisassembler()
    {
        ...
        rootDecoder = WideSparse(56, 8, "SX-Aurora", Instr(Mnemonic.Invalid, InstrClass.Invalid),
