...
internal class TriCoreInstructionComparer : InstructionComparer
{
    private const int MemCode = 0x10;

    public TriCoreInstructionComparer(Normalize norm) : base(norm)
    {
    }
    ...
    private bool CompareMemoryOperands(MemoryOperand m1, MemoryOperand op2)
    {
        ...
        if (!CompareConstants(m1.Offset, op2.Offset))
            return false;
        return
            m1.PreIncrement == op2.PreIncrement &&
    ...
    private int GetMemoryHash(MemoryOperand m)
    {
        ...
        hash = hash * 3 ^ GetConstantHash(m.Offset);
        hash = hash ^ (m.PreIncrement ? 2 : 0);
        hash = hash ^ (m.PostIncrement ? 1 : 0);
        return hash;
    }
}