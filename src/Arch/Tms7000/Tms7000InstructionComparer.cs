...
internal class Tms7000InstructionComparer : InstructionComparer
{
    private const int MemCode = 0x10;

    public Tms7000InstructionComparer(Normalize norm) : base(norm)
    {
    }
    ...
    {
        if (!CompareRegisters(m1.Register, m2.Register))
            return false;
        if (m1.Address is null)
            return m2.Address is null;
        if (m2.Address is null)
            return false;
        return CompareAddresses(m1.Address.Value, m2.Address.Value);
    }
    ...
    public override int GetOperandHash(MachineOperand op)
    {
        return op switch
        {
            ...
            MemoryOperand m => GetMemoryAccessHash(m),
            _ => throw new NotImplementedException(op.GetType().Name)
        };
    }

    private int GetMemoryAccessHash(MemoryOperand m)
    {
        int hash = MemCode;
        hash = hash * 17 ^ GetRegisterHash(m.Register);
        if (m.Address is not null)
            hash = hash * 17 ^ GetAddressHash(m.Address.Value);
        return hash;
    }
}