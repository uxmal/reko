...
public class C33InstructionComparer : InstructionComparer
{
    public C33InstructionComparer(Normalize norm) :
        base(norm)
    {
    }
    ...
    {
        if (!base.CompareOperands(m1.Base, m2.Base))
            return false;
        if (!base.CompareOperands(m1.Offset, m2.Offset))
            return false;
    ...
    {
        int h = GetOperandHash(m.Base);
        h ^= m.Offset is not null
            ? 17 * GetOperandHash(m.Offset)
            : 0;
        h ^= m.PostIncrement ? 0x999 : 0;
        return h;
    }
}