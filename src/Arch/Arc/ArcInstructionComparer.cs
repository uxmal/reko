...
{
    public class ArcInstructionComparer : InstructionComparer
    {
        private const int MemCode = 0x10;

        public ArcInstructionComparer(Normalize norm) : base(norm)
        {
        }
        ...
        private bool CompareMemoryOperand(MemoryOperand m1, MemoryOperand m2)
        {
            if (!CompareRegisters(m1.Base, m2.Base))
                return false;
            if (!base.CompareConstants(m1.Offset, m2.Offset))
                return false;
        ...
        private int GetMemoryOperandHash(MemoryOperand m)
        {
            ...
            hash = hash * 23 ^ GetConstantHash(m.Offset);
