...
internal class PaRiscInstructionComparer : InstructionComparer
{
    private const int MemCode = 0x10;

    public PaRiscInstructionComparer(Normalize norm) : base(norm)
    {
    }
    ...
    public override bool DoCompareOperands(MachineOperand op1, MachineOperand op2)
    {
        return op1 switch
        {
            ...
            LeftImmediateOperand l1 => CompareLeftImmediateOperands(l1, (LeftImmediateOperand) op2),
    ...
    private bool CompareMemoryOperands(MemoryOperand m1, MemoryOperand m2)
    {
        ...
        if (!CompareRegisters(m1.Index, m2.Index))
            return false;
        return CompareRegisters(m1.Space, m2.Space);
    }

    private bool CompareLeftImmediateOperands(LeftImmediateOperand l1, LeftImmediateOperand l2)
    {
        return CompareConstants(l1.Value, l2.Value);
    }
    ...
    public override int GetOperandHash(MachineOperand op)
    {
        return op switch
        {
            ...
            LeftImmediateOperand l => GetConstantHash(l.Value),
    ...
    private int GetMemoryHash(MemoryOperand m)
    {
        ...
        hash = hash * 17 ^ GetRegisterHash(m.Space);
        return hash;
    }
}