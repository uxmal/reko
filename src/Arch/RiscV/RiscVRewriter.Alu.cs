...
namespace Reko.Arch.RiscV
{
    public partial class RiscVRewriter
    {
        ...
        {
            var src1 = RewriteOp(instr.Opcodes__1);
            var src2 = RewriteOp(instr.Opcodes__2);
        ...
        private void RewriteAddi16sp()
        {
            ...
            var imm = ((ImmediateOperand) instr.Opcodes__0).Value.ToInt32();
        ...
        private void RewriteAddi4spn()
        {
            ...
            var imm = ((ImmediateOperand) instr.Opcodes__1).Value.ToInt32();
            var dst = RewriteOp(instr.Opcodes__0);
        ...
        private void RewriteAddw()
        {
            ...
            var src1 = RewriteOp(instr.Opcodes__1);
            Expression src2;
            if (instr.Opcodes__2 is ImmediateOperand imm2)
            {
                src2 = Constant.Int32(imm2.Value.ToInt32());
            }
            else
            {
                src2 = RewriteOp(instr.Opcodes__2);
            }
        ...
        private void RewriteLoad(DataType dt)
        {
            ...
            {
                var baseReg = RewriteOp(instr.Opcodes__1);
                var offset = ((ImmediateOperand) instr.Opcodes__2).Value;
                ea = baseReg;
                if (!offset.IsZero)
                {
                    ea = m.IAdd(ea, offset);
        ...
        {
            var dst = RewriteOp(instr.Opcodes__0);
            var ui = ((ImmediateOperand)instr.Opcodes__1).Value;
        ...
        {
            var src = RewriteOp(instr.Opcodes__0);
            if (instr.Opcodes__1 is MemoryOperand mem)
            {
            ...
            {
                var baseReg = RewriteOp(instr.Opcodes__1);
                var offset = ((ImmediateOperand) instr.Opcodes__2).Value.ToInt32();
        ...
        private void RewriteSxsp(DataType dt)
        {
            ...
            var offset = ((ImmediateOperand) instr.Opcodes__1).Value.ToInt32();
            RewriteStore(dt, baseReg, offset, src);
        }
