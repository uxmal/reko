...
internal class MilStd1750InstructionComparer : InstructionComparer
{
    private const int MemCode = 0x10;

    public MilStd1750InstructionComparer(Normalize norm) : base(norm)
    {
    }
    ...
    private bool CompareMemoryOperands(MemoryOperand m1, MemoryOperand m2)
    {
        ...
        if (!CompareStorages(m1.Index, m2.Index))
            return false;
        return m1.Indirected == m2.Indirected;
    }
    ...
    {
        int hash = MemCode;
        hash = hash * 31 + GetConstantHash(m.Displacement);
        hash = hash * 17 ^ GetRegisterHash(m.Index);
        hash = hash * 5 ^ (m.Indirected ? 1 : 0);
        return hash;
    }
}