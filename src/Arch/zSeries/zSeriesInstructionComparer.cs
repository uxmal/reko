...
internal class zSeriesInstructionComparer : InstructionComparer
{
    private const int MemCode = 0x10;

    public zSeriesInstructionComparer(Normalize norm) : base(norm)
    {
    }
    ...
    private bool CompareMemoryOperands(MemoryOperand m1, MemoryOperand m2)
    {
        ...
        if (!CompareRegisters(m1.Index, m2.Index))
            return false;
        if (!CompareConstants(m1.Length, m2.Length))
            return false;
    ...
    private int GetMemoryHash(MemoryOperand m)
    {
        ...
        hash = hash * 17 ^ GetConstantHash(m.Length);
