...
internal class BeyondInstructionComparer : InstructionComparer
{
    private const int MemCode = 0x10;

    public BeyondInstructionComparer(Normalize norm) : base(norm)
    {
    }
    ...
    private bool CompareMemoryOperand(MemoryOperand m1, MemoryOperand m2)
    {
        if (!CompareRegisters(m1.Base, m2.Base))
            return false;
        return NormalizeConstants || m1.Offset == m2.Offset;
    }
