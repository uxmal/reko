...
namespace Reko.Arch.Pdp.Memory
{
    ...
    /// </summary>
    public class Word18MemoryArea : MemoryArea
    {
        public Word18MemoryArea(Address addrBase, uint[] words)
            : base(addrBase, words.Length, 18, new Word36MemoryFormatter(4, 6, 5))
        {
            Words = words;
        }
        ...
        public override EndianImageReader CreateBeReader(Address addr) => new Word18BeImageReader(this, addr);
        public override EndianImageReader CreateBeReader(Address addr, long cUnits) => new Word18BeImageReader(this, addr, cUnits);
        public override EndianImageReader CreateBeReader(long offset) => new Word18BeImageReader(this, offset);
        public override EndianImageReader CreateBeReader(long beginOffset, long endOffset) => new Word18BeImageReader(this, beginOffset, endOffset);
        ...
        {
            return string.Format("Image {0}{1} - length {2} 36-bit words {3}", "{", BaseAddress, Length, "}");
        }
        ...
        public override bool TryReadBe(long imageOffset, DataType type, out Constant c)
        {
            ...
            {
                c = Constant.Create(PdpTypes.Word36, Words[imageOffset]);
                return true;
            }
            c = default!;
            return false;
