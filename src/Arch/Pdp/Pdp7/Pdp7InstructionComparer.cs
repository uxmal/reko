...
internal class Pdp7InstructionComparer : InstructionComparer
{
    private const int MemCode = 0x10;

    public Pdp7InstructionComparer(Normalize norm) : base(norm)
    {
    }
    ...
    public override bool DoCompareOperands(MachineOperand op1, MachineOperand op2)
    {
        return op1 switch
        {
            ...
            _ => throw new NotImplementedException(op1.GetType().FullName)
        };
    }

    private bool CompareMemoryOperands(MemoryOperand m1, MemoryOperand m2)
    {
        if (!CompareConstants(m1.Address, m2.Address))
            return false;
    ...
    {
        int hash = MemCode;
        hash = hash * 11 ^ GetConstantHash(m.Address);
        hash = hash * 7 ^ (m.Deferred ? 1 : 0);
        return hash;
    }
}