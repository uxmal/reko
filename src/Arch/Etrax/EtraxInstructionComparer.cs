...
internal class EtraxInstructionComparer : InstructionComparer
{
    private const int MemCode = 0x10;

    public EtraxInstructionComparer(Normalize norm) : base(norm)
    {
    }
    ...
    public override bool DoCompareOperands(MachineOperand op1, MachineOperand op2)
    {
        return op1 switch
        {
            ...
            FlagGroupStorage grf => CompareFlagGroups(grf, (FlagGroupStorage) op2),
    ...
    private bool CompareMemoryOperands(MemoryOperand m1, MemoryOperand m2)
    {
        ...
        if (!CompareOperands(m1.Offset, m2.Offset))
            return false;
        if (m1.PostIncrement != m1.PostIncrement)
            return false;
        return true;
    }
    ...
    {
        int hash = MemCode;
        hash = hash * 17 ^ GetOperandHash(m.Base);
        hash = hash * 17 ^ GetOperandHash(m.Offset);
        hash = hash * 2 ^ (m.PostIncrement ? 1 : 0);
        return hash;
    }
}
