...
internal class M6812InstructionComparer : InstructionComparer
{
    private const int MemCode = 0x10;

    public M6812InstructionComparer(Normalize norm) : base(norm)
    {
    }
    ...
    private bool CompareMemoryOperands(MemoryOperand m1, MemoryOperand m2)
    {
        ...
        if (!CompareRegisters(m1.Index, m2.Index))
            return false;
        if (m1.Offset.HasValue)
        {
            if (!m2.Offset.HasValue)
                return false;
            if (!CompareConstants(m1.Offset.Value, m2.Offset.Value))
                return false;
        }
        else
        {
            return !m2.Offset.HasValue;
        }
        return m1.PreIncrement == m2.PreIncrement &&
               m1.PostIncrement == m2.PostIncrement &&
               m1.Indirect == m2.Indirect;
    }
    ...
    private int GetMemoryHash(MemoryOperand m)
    {
        ...
        hash = hash * 31 ^ (m.Offset.HasValue ? m.Offset.Value : 0);
        hash = hash * 5;
        hash = hash ^ (m.PreIncrement ? 4 : 0);
        hash = hash ^ (m.PostIncrement ? 2 : 0);
        hash = hash ^ (m.Indirect ? 1 : 0);
        return hash;
    }
}