...
namespace Reko
{
	...
	public class Decompiler : IDecompiler
	{
        ...
        /// </summary>
        public void Decompile(ImageLocation imageUri, string? loaderName = null)
        {
            try
            {
                ExtractResources();
        ...
        public void Assemble(ImageLocation fileUri, IAssembler asm, IPlatform platform)
        {
        }


        [Obsolete("", true)]
        protected Project AddProgramToProject(ImageLocation absoluteUri, Program program)
        {
        ...
        private void WriteEntryPoints(Program program)
        {
            if (program.Platform == null)
                return;
            var irPath = Path.Combine(program.SourceDirectory, Path.GetFileNameWithoutExtension(program.Uri.FilesystemPath));
            program.Platform.WriteMetadata(program, irPath);
        }

		public void WriteHeaderComment(string filename, Program program, TextWriter w)
		{
            var fragment = program.Uri.FilesystemPath;
        ...
        {
            var filename = Path.ChangeExtension(program.Uri.GetFilename(), "sccs");
